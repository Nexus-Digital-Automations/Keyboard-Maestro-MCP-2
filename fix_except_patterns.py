...
PROJECT_ROOT = Path("/Users/jeremyparker/Desktop/Claude Coding Projects/Keyboard-Maestro-MCP/src")
...
def detect_context(code_block: str) -> str:
    ...
    if any(keyword in code_lower for keyword in ['open(', 'file', 'path', '.read(', '.write(']):
        return 'file_ops'
    elif any(keyword in code_lower for keyword in ['requests.', 'http', 'url', 'fetch']):
        return 'network'
    elif any(keyword in code_lower for keyword in ['json.', '.loads(', '.dumps(']):
        return 'json_ops'
    elif any(keyword in code_lower for keyword in ['pickle.', '.loads(', '.dumps(']):
        return 'pickle_ops'
    elif any(keyword in code_lower for keyword in ['int(', 'float(', 'str(', 'bool(']):
        return 'type_ops'
...
def fix_except_patterns(file_path: Path) -> Tuple[bool, int]:
    ...
    try:
        ...
        # Pattern 1: except: pass (including variations)
        pattern1 = re.compile(r'(\s+)except\s*:\s*(?:\n\s*#[^\n]*)?\n(\s+)pass\s*(?:\n|$)', re.MULTILINE)
        ...
        content = pattern1.sub(replace_except_pass, content)
        
        # Pattern 2: except: continue
        pattern2 = re.compile(r'(\s+)except\s*:\s*\n(\s+)continue\s*(?:\n|$)', re.MULTILINE)
        
        def replace_except_continue(match):
            nonlocal changes_made
            indent = match.group(1)
        ...
        content = pattern2.sub(replace_except_continue, content)
        
        # Pattern 3: except: return (something)
        pattern3 = re.compile(r'(\s+)except\s*:\s*\n(\s+)(return\s+[^\n]*)\s*(?:\n|$)', re.MULTILINE)
        
        def replace_except_return(match):
            nonlocal changes_made
            indent = match.group(1)
            ...
            changes_made += 1
            return f'{indent}except {exceptions} as e:\n{return_indent}logger.debug(f"{log_msg}: {{e}}")\n{return_indent}{return_statement}\n'
        
        content = pattern3.sub(replace_except_return, content)
        
        # Pattern 4: except Exception as e: followed by pass or minimal handling
        pattern4 = re.compile(r'(\s+)except\s+Exception\s+as\s+\w+\s*:\s*\n(\s+)pass\s*(?:\n|$)', re.MULTILINE)
        
        def replace_except_exception_pass(match):
            nonlocal changes_made
            indent = match.group(1)
            pass_indent = match.group(2)
        ...
        content = pattern4.sub(replace_except_exception_pass, content)
        ...
        if changes_made > 0:
            ...
            if 'import logging' not in content and 'from logging import' not in content:
                ...
                if import_pattern:
                    ...
                    logger_setup = '\nimport logging\n\nlogger = logging.getLogger(__name__)\n'
                    content = content[:imports_end] + logger_setup + content[imports_end:]
                else:
                    ...
                    logger_setup = f'\nimport logging\n\nlogger = logging.getLogger(__name__)\n'
                    content = content[:docstring_end] + logger_setup + content[docstring_end:]
        ...
        if py_file.is_file():
            try:
                ...
                if modified:
                    ...
                    logger.info(f"Fixed {changes} patterns in {py_file.relative_to(PROJECT_ROOT)}")
            except Exception as e:
...
def main():
    ...
    logger.info(f"Bulk fix completed!")
