...
def fix_remaining_return_types(file_path: Path) -> tuple[bool, int]:
    ...
    try:
        ...
        for pattern in patterns:
            def replace_function_def(match: Any) -> Any:
                ...
                if any(pattern in func_name.lower() for pattern in [
                    'test_', 'setup', 'teardown', 'init', 'configure', 'register',
                    'unregister', 'start', 'stop', 'cleanup', 'clear', 'reset',
                    'update', 'add', 'remove', 'delete', 'create', 'destroy',
                    'save', 'load', 'write', 'send', 'receive', 'connect',
                    'disconnect', 'bind', 'unbind', 'handle', 'process',
                    'execute', 'run', 'perform', 'trigger', 'activate',
                    'deactivate', 'enable', 'disable', 'cancel', 'pause',
                    'resume', 'close', 'open', 'quit', 'exit', 'validate',
                    'verify', 'check', 'ensure', 'log', 'print', 'debug',
                    'info', 'warn', 'error'
                ]):
                    return_type = "-> None:"
                elif any(pattern in func_name.lower() for pattern in [
                    'is_', 'has_', 'can_', 'should_', 'will_', 'was_', 'were_',
                    'contains', 'includes', 'exists', 'matches', 'equals'
                ]):
                    return_type = "-> bool:"
                elif any(pattern in func_name.lower() for pattern in [
                    'get_', 'format_', 'generate_', 'build_', 'render_',
                    'stringify', 'serialize'
                ]):
                    return_type = "-> Any:"
                elif any(pattern in func_name.lower() for pattern in [
                    'list_', 'find_', 'search_', 'collect_', 'gather_',
                    'filter_', 'map_', 'sort_'
                ]):
                    return_type = "-> list[Any]:"
                elif any(pattern in func_name.lower() for pattern in [
                    'dict_', 'config_', 'settings_', 'options_', 'params_',
                    'metadata_', 'data_', 'result_', 'response_'
                ]):
        ...
        if changes_made > 0:
            if not any(import_line in content for import_line in [
...
def fix_remaining_argument_types(file_path: Path) -> tuple[bool, int]:
    ...
    try:
        ...
        for line_no, func_name, node in functions_to_fix:
            ...
            for arg in node.args.args:
                if arg.annotation is None and arg.arg not in ['self', 'cls']:
                    ...
                    if 'context' in arg.arg.lower() or 'ctx' in arg.arg.lower():
                        arg_type = 'Any'
                    elif 'mock' in arg.arg.lower():
                        arg_type = 'Any'
                    elif arg.arg in ['data', 'content', 'value']:
                        arg_type = 'Any'
        ...
        if py_file.is_file() and not py_file.name.startswith('.'):
            try:
                ...
                if total_file_changes > 0:
                    ...
                    logger.info(f"Fixed {total_file_changes} annotations in {py_file.relative_to(PROJECT_ROOT)}")
