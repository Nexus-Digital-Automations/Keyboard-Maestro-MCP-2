...
def infer_argument_type(arg_name: str, func_content: str, func_name: str) -> str:
    ...
    if f"len({arg_name})" in func_content or (
        "for " in func_content and f" in {arg_name}" in func_content
    ):
        return "list[Any] | str"
    ...
    try:
        ...
        for func_node in functions_to_fix:
            ...
            for arg in func_node.args.args:
                if arg.annotation is None and arg.arg not in ["self", "cls"]:
                    inferred_type = infer_argument_type(
                        arg.arg,
                        func_content,
                        func_name,
                    )
        ...
        if changes_made > 0 and not has_typing_import:
            ...
            if "from __future__ import annotations" in content:
                ...
                if import_pattern:
                    ...
                    content = (
                        content[:imports_start] + import_text + content[imports_start:]
                    )
                else:
        ...
        if py_file.is_file() and not py_file.name.startswith("."):
            try:
                ...
                if modified:
                    ...
                    logger.info(
                        f"Fixed {changes} argument annotations in "
