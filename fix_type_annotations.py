...
This script systematically adds missing type annotations for ANN201 and ANN001
violations across the codebase for improved code quality and maintainability.
"""
...
RETURN_TYPE_PATTERNS = {
    ...
    (
        r"def\s+(?:test_|example_|main|setup|teardown|init|clear|reset|update|"
        r"add|remove|delete|create|register|unregister|start|stop|cleanup|"
        r"configure|initialize|finalize|validate|check|verify|ensure|log|"
        r"print|debug|info|warn|error|critical|handle|process|execute|run|"
        r"perform|trigger|activate|deactivate|enable|disable|cancel|pause|"
        r"resume|close|open|quit|exit|save|load|write|read|set|get|put|post|"
        r"send|receive|connect|disconnect|bind|unbind|attach|detach|sync|"
        r"async|await|yield|return|raise|throw|catch|try|finally|with|as|"
        r"import|from|pass|break|continue|while|for|if|else|elif|assert|del|"
        r"global|nonlocal|lambda|class|def|async|await)[^(]*\([^)]*\)\s*:\s*$"
    ): "-> None:",
    # Functions that return boolean
    (
        r"def\s+(?:is_|has_|can_|should_|will_|was_|were_|contains|includes|"
        r"exists|matches|equals|compare|validate|verify|check|test|confirm|"
        r"ensure)[^(]*\([^)]*\)\s*:\s*$"
    ): "-> bool:",
    # Functions that return strings
    (
        r"def\s+(?:get_|format_|generate_|create_|build_|render_|stringify|"
        r"to_string|as_string|serialize)[^(]*\([^)]*\)\s*:\s*$"
    ): "-> str:",
    # Functions that return lists
    (
        r"def\s+(?:list_|find_|search_|collect_|gather_|filter_|map_|sort_|"
        r"group_)[^(]*\([^)]*\)\s*:\s*$"
    ): "-> list[Any]:",
    # Functions that return dictionaries
    (
        r"def\s+(?:dict_|config_|settings_|options_|params_|metadata_|info_|"
        r"data_|result_|response_|status_|state_)[^(]*\([^)]*\)\s*:\s*$"
    ): "-> dict[str, Any]:",
}
...
def detect_return_type(func_content: str, func_name: str) -> str:
    ...
    if re.search(
        r"return\s+(True|False|\w+\s*==\s*\w+|\w+\s*!=\s*\w+|\w+\s*>\s*\w+|\w+\s*<\s*\w+|\w+\s*>=\s*\w+|\w+\s*<=\s*\w+|\w+\s*and\s*\w+|\w+\s*or\s*\w+|not\s+\w+|\w+\s*in\s*\w+|\w+\s*is\s*\w+|\w+\s*is\s*not\s*\w+)",
        func_content,
    ):
        return "-> bool:"
...
def detect_argument_type(arg_name: str, func_content: str) -> str:
    ...
    if f"len({arg_name})" in func_content or (
        "for" in func_content and arg_name in func_content
    ):
        return "list[Any]"
        ...
        if changes_made > 0 and not has_typing_import:
            ...
            if "from __future__ import annotations" in content:
                ...
                import_pattern = re.search(
                    r"((?:^(?:from __future__|from|import)\s+[^\n]+\n)+)",
                    content,
                    re.MULTILINE,
                )
                if import_pattern:
                ...
                if import_pattern:
                    ...
                    imports = "\nfrom __future__ import annotations\n\nfrom typing import Any, Optional\n"
                    content = (
                        content[:docstring_end] + imports + content[docstring_end:]
                    )
...
def process_directory(
    ...
    if target_files:
        files_to_process = [
            directory / f for f in target_files if (directory / f).exists()
        ]
    else:
        ...
        if py_file.is_file() and not py_file.name.startswith("."):
            try:
                ...
                if modified:
                    ...
                    logger.info(
                        f"Fixed {changes} annotations in {py_file.relative_to(PROJECT_ROOT)}",
                    )
            except Exception as e:
