...
class AgentState:
    ...
    def get_priority_goal(self) -> AgentGoal | None:
...
class AutonomousAgent:
    ...
    async def _execution_loop(self) -> None:
        ...
        while (
            ...
            and self.state.status == AgentStatus.ACTIVE
        ):
            try:
                # Execute one cycle
                ...
                if cycle_result.is_left():
                    ...
                    interval = (
                        self.state.configuration.monitoring_interval.total_seconds()
                    )
    ...
    async def _learn_from_action(
        self,
        action: AgentAction,
        result: Either[AutonomousAgentError, dict[str, Any]],
        ...
        try:
            ...
            outcome = (
                result.get_right() if success else {"error": str(result.get_left())}
            )
    ...
    async def _perform_self_optimization(self) -> None:
        ...
        try:
            ...
            recent_experiences = (
                self.state.experiences[-50:]
                if len(self.state.experiences) >= 50
                else self.state.experiences
            )
    ...
    def _is_goal_achieved(self, goal: AgentGoal) -> bool:
        ...
        recent_actions = [
            a for a in self.state.completed_actions if a.startswith(goal.goal_id[:8])
        ]
    ...
    def _check_goal_conflicts(self, new_goal: AgentGoal) -> list[str]:
        ...
        for existing_goal in self.state.current_goals:
            ...
            if (
                new_goal.priority == GoalPriority.EMERGENCY
                and existing_goal.priority == GoalPriority.EMERGENCY
            ):
    ...
    def _validate_configuration(self) -> Either[AutonomousAgentError, None]:
        ...
        # Validate thresholds
        if (
            config.decision_threshold > 0.95
            and config.autonomy_level == AutonomyLevel.AUTONOMOUS
        ):
            return Either.left(
        ...
        try:
            if message.message_type == MessageType.HELP_REQUEST:
                ...
                if self.resource_optimizer:
                    ...
                    can_help = all(
                        available.get(res, 0)
                        > amount * 1.5  # Have 50% more than needed
                ...
                if self.communication_hub:
                    await self.communication_hub.acknowledge_message(
                        ...
                        {
                            "status": "considering",
                            "agent_type": self.state.configuration.agent_type.value,
                        },
                    )
