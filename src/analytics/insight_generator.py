...
class InsightGenerator:
    ...
    async def _generate_efficiency_insights(
        ...
        if (
            "workflow_steps" in data.metrics
            and "average_completion_time" in data.metrics
        ):
    ...
    def _estimate_performance_roi(self, response_time: float) -> float:
        ...
        (response_time - 2.0) / response_time
    ...
    def _calculate_performance_roi(self, current_time: float) -> dict[str, float]:
        ...
        target_time = 2.0
        max(0, (current_time - target_time) / current_time)
        ...
        roi_percentage = (
            (annual_benefit - implementation_cost) / implementation_cost
        ) * 100
        payback_months = (
            (implementation_cost / annual_benefit) * 12 if annual_benefit > 0 else 999
        )
    ...
    def _calculate_cost_savings_roi(self, potential_savings: float) -> dict[str, float]:
        ...
        roi_percentage = (
            (potential_savings - implementation_cost) / implementation_cost
        ) * 100
        payback_months = (
            (implementation_cost / potential_savings) * 12
    ...
    async def generate_executive_summary(
        ...
        try:
            ...
            overall_roi = (
                (
                    (total_potential_savings - total_investment_required)
                    / total_investment_required
                    * 100
                )
                if total_investment_required > 0
                else 0
            )
            ...
            # Overall confidence
            confidence_score = (
                statistics.mean([i.confidence_score for i in insights])
                if insights
                else 0.0
            )
