...
class ModelManager:
    ...
    async def _prepare_training_data(
        ...
        try:
            ...
            missing_features = [
                col for col in config.feature_columns if col not in dataset.feature_data
            ]
            if missing_features:
                return Either.left(
                    ModelTrainingError(
            ...
            features = {
                col: dataset.feature_data[col] for col in config.feature_columns
            }
            ...
            train_data = {
                "features": {
                    col: values[:train_end] for col, values in features.items()
                },
                "targets": targets[:train_end],
                "timestamps": dataset.timestamps[:train_end]
            ...
            val_data = {
                "features": {
                    col: values[train_end:val_end] for col, values in features.items()
                },
                "targets": targets[train_end:val_end],
                "timestamps": dataset.timestamps[train_end:val_end]
                if dataset.timestamps
                else None,
            }
            ...
            test_data = {
                ...
                "timestamps": dataset.timestamps[val_end:]
                if dataset.timestamps
                else None,
            }
    ...
    async def _validate_time_series_split(
        ...
        try:
            ...
            is_passed = (
                validation_score >= 0.65 and mape <= 15.0
            )  # More lenient for time series
            failure_reasons = []
    ...
    def _calculate_rmse(
        ...
        numeric_pairs = [
            (float(t), float(p))
            for t, p in zip(targets, predictions, strict=False)
    ...
    async def _save_model_artifact(
        ...
        try:
            ...
            model_filename = (
                f"{config.model_id}_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}.pkl"
            )
    ...
    async def get_model_status(self, model_id: ModelId) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "created_at": training_history[0]["timestamp"]
                if training_history
                else None,
                "performance": {
                ...
                "last_training": training_history[-1]["timestamp"]
                if training_history
                else None,
                "deployment_info": {
    ...
    async def get_model_manager_summary(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "validation_methods_supported": [
                    method.value for method in ValidationMethod
                ],
