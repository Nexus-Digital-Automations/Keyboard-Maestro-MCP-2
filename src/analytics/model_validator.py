...
class ModelValidator:
    ...
    def _initialize_validation_templates(self) -> None:
        ...
        self.validation_templates[ModelType.TIME_SERIES_FORECASTING] = (
    ...
    async def validate_model(
        ...
        try:
            ...
            # Check cache
            cache_key = (
                f"{config.model_id}_{hash(str(config))}_{hash(str(dataset.features))}"
            )
            ...
            self.validation_history.append(
                {
                    ...
                    "validation_methods": [
                        method.value for method in config.validation_methods
                    ],
                    "overall_score": overall_score,
    ...
    async def _holdout_validation(
        ...
        residuals = [
            actual - pred
            for actual, pred in zip(test_targets, predictions, strict=False)
        ]
    ...
    async def _k_fold_validation(
        ...
        for fold in range(config.k_folds):
            ...
            [i for i in indices if i not in test_indices]
        ...
        residuals = [
            actual - pred
            for actual, pred in zip(all_actuals, all_predictions, strict=False)
        ]
            ...
            for i, result1 in enumerate(results):
                for j, result2 in enumerate(results):
                    if i < j:
                        ...
                        if (
                            primary_metric in result1.validation_metrics
                            and primary_metric in result2.validation_metrics
                        ):
                            ...
                            pooled_std = (
                                ...
                                / 2
                                if len(result1.residuals) > 1
                                and len(result2.residuals) > 1
                                else 0.0
                            )

                            t_statistic = difference / max(0.001, pooled_std)
                            p_value = 2 * (
                                1 - min(0.999, t_statistic / 10)
                            )  # Simplified p-value
        ...
        for metric, values in metric_variations.items():
            if len(values) > 1:
                ...
                coefficient_of_variation = (
                    std_value / abs(mean_value) if mean_value != 0 else float("inf")
                )
                ...
                stability_analysis[metric.value] = {
                    ...
                    "relative_range": (max(values) - min(values)) / abs(mean_value)
                    if mean_value != 0
                    else 0.0,
                }
    ...
    def _generate_validation_recommendations(
        ...
        if cross_validation_scores:
            for method, method_scores in cross_validation_scores.items():
                for _metric, scores in method_scores.items():
                    if len(scores) > 1:
                        cv_std = statistics.stdev(scores)
                        cv_mean = statistics.mean(scores)
                        cv_coefficient = (
                            cv_std / abs(cv_mean) if cv_mean != 0 else float("inf")
                        )
    ...
    def _calculate_overall_validation_score(
        ...
        if model_type == ModelType.REGRESSION:
            ...
            metric_weights = dict.fromkeys(aggregated_metrics.keys(), 1.0)
    ...
    def _calculate_confidence_intervals(
        ...
        return {
            "residual_range": (
                sorted_residuals[lower_index],
                sorted_residuals[upper_index],
            ),
            "mean_confidence": (
    ...
    def _calculate_mae(self, actuals: list[float], predictions: list[float]) -> float:
        ...
        if len(actuals) != len(predictions) or len(actuals) == 0:
            return float("inf")
        return sum(
            abs(a - p) for a, p in zip(actuals, predictions, strict=False)
        ) / len(actuals)
    ...
    def _calculate_mse(self, actuals: list[float], predictions: list[float]) -> float:
        ...
        if len(actuals) != len(predictions) or len(actuals) == 0:
            return float("inf")
        return sum(
            (a - p) ** 2 for a, p in zip(actuals, predictions, strict=False)
        ) / len(actuals)
    ...
    def _calculate_forecast_bias(
        ...
        errors = [p - a for a, p in zip(actuals, predictions, strict=False)]
    ...
    def _calculate_prediction_stability(
        ...
        prediction_changes = [
            abs(predictions[i] - predictions[i - 1]) for i in range(1, len(predictions))
        ]
        actual_changes = [
            abs(actuals[i] - actuals[i - 1]) for i in range(1, len(actuals))
        ]
    ...
    def _calculate_model_confidence(
        ...
        pred_variance = (
            statistics.variance(predictions) if len(predictions) > 1 else 0.0
        )
    ...
    async def get_validation_metrics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "most_validated_model_types": [
                model_type for model_type, count in Counter(model_types).most_common(3)
            ],
            "validation_success_rate": len([s for s in scores if s > 0.6]) / len(scores)
            if scores
            else 0.0,
        }
