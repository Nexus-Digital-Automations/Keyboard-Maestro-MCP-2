...
class OptimizationModeler:
    ...
    async def generate_optimization_recommendations(
        ...
        try:
            ...
            solutions.sort(
                key=lambda s: (s.confidence_score, s.improvement_percentage),
                reverse=True,
            )
    ...
    async def _solve_optimization_problem(
        ...
        try:
            ...
            improvement_percentage = (
                (objective_value - baseline_value) / baseline_value
            ) * 100
    ...
    async def _analyze_trade_offs(
        ...
        for var_name, var_value in optimized_variables.items():
            if isinstance(var_value, int | float):
                change_factor = (
                    var_value if isinstance(var_value, float) else float(var_value)
                )
    ...
    def _assess_implementation_complexity(
        ...
        for var_name, var_value in optimized_variables.items():
            if isinstance(var_value, int | float):
                ...
                change_magnitude = (
                    abs(var_value - 1.0) if isinstance(var_value, float) else 0.1
                )
                ...
                if any(
                    ...
                    for keyword in ["architecture", "algorithm", "protocol"]
                ):
        ...
        for metric_name, values in outcomes.items():
            if values:
                summary[metric_name] = {
                    ...
                    "min": min(values),
                    "max": max(values),
                    "percentile_25": statistics.quantiles(values, n=4)[0]
                    if len(values) >= 4
                    else min(values),
                    "percentile_75": statistics.quantiles(values, n=4)[2]
                    if len(values) >= 4
                    else max(values),
                }
        ...
        for metric_name, values in outcomes.items():
            if values:
                ...
                if num_bins > 0:
                    ...
                    bin_size = (
                        (max_val - min_val) / num_bins if max_val > min_val else 1.0
                    )
        ...
        for var_name, var_value in optimization_solution.optimized_variables.items():
            if isinstance(var_value, int | float):
                ...
                sensitivity[var_name] = (
                    abs(outcome_change / delta) if delta != 0 else 0.0
                )
    ...
    async def generate_trade_off_analysis(
        ...
        try:
            ...
            # Extract objectives
            objectives = list({solution.optimization_target for solution in solutions})
        ...
        for solution in solutions:
            ...
            for other_solution in solutions:
                if other_solution != solution:
                    other_point = (
                        other_solution.improvement_percentage,
                        other_solution.confidence_score,
                    )
                    if (
        ...
        for i, obj1 in enumerate(objectives):
            for j, obj2 in enumerate(objectives):
                if i < j:  # Avoid duplicate pairs
                    ...
                    obj1_solutions = [
                        s for s in solutions if s.optimization_target == obj1
                    ]
                    obj2_solutions = [
                        s for s in solutions if s.optimization_target == obj2
                    ]
    ...
    def _recommend_optimal_balance(
        ...
        sum(solution.confidence_score for solution in solutions)

        for objective in objectives:
            objective_solutions = [
                s for s in solutions if s.optimization_target == objective
            ]
            ...
            if objective_solutions:
                ...
                if weighted_confidence > 0:
                    balance[objective.value] = (
                        weighted_improvement / weighted_confidence
                    )
    ...
    async def _analyze_balance_sensitivity(
        ...
        for solution in solutions:
            ...
            trade_off_magnitude = sum(
                abs(value) for value in solution.trade_offs.values()
            )
    ...
    def _build_decision_matrix(
        ...
        for solution in solutions:
            ...
            avg_risk = (
                statistics.mean(solution.risk_assessment.values())
                if solution.risk_assessment
                else 0.5
            )
    ...
    async def get_optimization_performance_metrics(self) -> dict[str, Any]:
        ...
        recent_optimizations = list(self.optimization_history)[
            -50:
        ]  # Last 50 optimizations

        improvements = [
            opt["best_improvement"]
            for opt in recent_optimizations
            if opt["best_improvement"] > 0
        ]
        ...
        return {
            ...
            "success_rate": len(improvements) / len(recent_optimizations)
            if recent_optimizations
            else 0.0,
            "cache_hit_rate": len(self.solution_cache)
            / max(1, len(self.optimization_history)),
            "most_optimized_targets": [
                target
                for target, count in Counter(
                    opt["optimization_target"] for opt in recent_optimizations
                ).most_common(3)
            ],
        }
