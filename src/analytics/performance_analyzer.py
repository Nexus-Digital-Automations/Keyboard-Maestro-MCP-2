...
from src.core.predictive_modeling import create_model_id
from src.core.types import ToolId
...
class PerformanceAnalyzer:
    ...
    def __init__(self, analytics_config: dict[str, Any] | None = None):
    ...
    ) -> Either[ValidationError, AnalysisResult]:
    ...
    def _detect_degradation(self, values: list[float]) -> dict[str, Any] | None:
    ...
    async def _identify_optimizations(
        ...
        for metric_id, values in metrics.items():
            ...
            numeric_values = [
                float(v.value) for v in values if isinstance(v.value, int | float)
            ]
            if not numeric_values:
                continue
            ...
            if len(numeric_values) > 1:
                ...
                if (
                    mean_val > 0 and (variance / mean_val) > 0.5
    ...
    async def _calculate_performance_score(
        ...
        for metric_id, values in metrics.items():
            ...
            if metric_id in self.baselines:
                baseline = self.baselines[metric_id]
                recent_values = [
                    v for v in values[-10:] if isinstance(v.value, int | float)
                ]
    ...
    def _update_analysis_stats(
        ...
        new_avg = (
            current_avg * (total_analyses - 1) + analysis_time_ms
        ) / total_analyses
    ...
    async def get_analysis_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "model_accuracies": {
                name: model["accuracy"] for name, model in self.models.items()
            },
