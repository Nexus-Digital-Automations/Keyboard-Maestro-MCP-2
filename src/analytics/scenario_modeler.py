...
class ScenarioResults:
    ...
    def __post_init__(self):
        if (
            len(self.individual_outcomes)
            != self.scenario_configuration.simulation_iterations
        ):
...
class ScenarioModeler:
    ...
    async def _run_queue_theory_simulation(
        ...
        for iteration in range(config.simulation_iterations):
            ...
            if service_rate > arrival_rate:
                ...
                avg_queue_length = (arrival_rate**2) / (
                    service_rate * (service_rate - arrival_rate)
                )
                avg_wait_time = arrival_rate / (
                    service_rate * (service_rate - arrival_rate)
                )
            else:
                # Unstable queue
    ...
    def _generate_parameter_combinations(
        ...
        for param in parameters:
            if param.parameter_type == "numeric":
                min_val = (
                    ...
                    else float(param.base_value) * 0.5
                )
                max_val = (
                    param.max_value
                    if param.max_value is not None
                    else float(param.base_value) * 2.0
                )

                param_range = []
                for i in range(steps_per_param):
                    value = (
                        min_val + (max_val - min_val) * i / (steps_per_param - 1)
        ...
        while len(combinations) < num_combinations:
            param_values = {}
            for param in parameters:
                param_values[param.parameter_name] = (
                    self._generate_random_parameter_value(param)
                )
            combinations.append(param_values)
    ...
    def _generate_correlated_parameter_values(
        ...
        for param in parameters:
            parameter_values[param.parameter_name] = (
                self._generate_random_parameter_value(param)
            )
    ...
    def _calculate_time_factor(
        ...
        progress = (
            current_time.total_seconds() / total_time.total_seconds()
    ...
    async def _simulate_scenario_outcome(
        ...
        if config.scenario_type == ScenarioType.STRESS_TEST:
            ...
            parameter_values.get("Concurrent Users", 100)

            # Simulate performance degradation under load
            base_throughput = baseline.get("throughput", 100.0)
            outcome_metrics["throughput"] = (
                base_throughput
                * load_multiplier
                * (1 - 0.1 * math.log(load_multiplier) if load_multiplier > 1 else 1)
            )
            outcome_metrics["latency"] = baseline.get("latency", 200.0) * (
                1 + 0.5 * load_multiplier
            )
            ...
            outcome_metrics["resource_cost"] = (
                outcome_metrics["required_capacity"] * 0.01 / resource_efficiency
            )
            outcome_metrics["utilization_efficiency"] = (
            ...
            outcome_metrics["total_errors"] = (
                outcome_metrics["automated_tasks"] * error_rate
            )
            outcome_metrics["efficiency_gain"] = (
                automation_rate * 0.8
            )  # 80% efficiency gain
            outcome_metrics["cost_savings"] = (
                automation_rate * baseline.get("cost", 10.0) * 0.6
            )
    ...
    def _calculate_cost_metrics(
        ...
        if "resource_utilization" in outcome_metrics:
            ...
            cost_metrics["infrastructure_cost"] = (
                utilization * 100.0
            )  # $100/hour at full utilization

        # Operational costs
        if "manual_tasks" in outcome_metrics:
            manual_tasks = outcome_metrics["manual_tasks"]
            cost_metrics["operational_cost"] = (
                manual_tasks * 0.5
            )  # $0.50 per manual task
        ...
        if "throughput" in outcome_metrics and outcome_metrics["throughput"] > 0:
            cost_metrics["cost_per_unit"] = (
                cost_metrics["total_cost"] / outcome_metrics["throughput"]
            )
        ...
        for metric_name, values in all_metrics.items():
            if values:
                ...
                if len(values) >= 10:
                    ...
                    summary[metric_name]["percentile_10"] = sorted_values[
                        len(sorted_values) // 10
                    ]
                    summary[metric_name]["percentile_25"] = sorted_values[
                        len(sorted_values) // 4
                    ]
                    summary[metric_name]["percentile_75"] = sorted_values[
                        3 * len(sorted_values) // 4
                    ]
                    summary[metric_name]["percentile_90"] = sorted_values[
                        9 * len(sorted_values) // 10
                    ]
    ...
    async def _perform_uncertainty_analysis(
        ...
        for param in config.parameters:
            ...
            param_values = [
                outcome.parameter_values.get(param_name, param.base_value)
                for outcome in outcomes
            ]

            if param_values and all(isinstance(v, int | float) for v in param_values):
                param_variance = (
                    statistics.variance(param_values) if len(param_values) > 1 else 0.0
                )
    ...
    def _assess_scenario_risks(
        ...
        # Collect key metrics
        success_rates = [
            outcome.outcome_metrics.get("simulation_success", 1.0)
            for outcome in outcomes
        ]
        error_rates = [
            outcome.outcome_metrics.get("error_rate", 0.0) for outcome in outcomes
        ]
        cost_metrics = [
            outcome.cost_metrics.get("total_cost", 0.0) for outcome in outcomes
        ]
    ...
    def _generate_scenario_recommendations(
        ...
        throughput_values = [
            outcome.outcome_metrics.get("throughput", 0.0) for outcome in outcomes
        ]
        ...
        if cost_values:
            ...
            cost_variance = (
                statistics.variance(cost_values) if len(cost_values) > 1 else 0.0
            )
        ...
        if config.scenario_type == ScenarioType.STRESS_TEST:
            ...
            utilization_values = [
                outcome.resource_utilization.get("cpu", 0.0) for outcome in outcomes
            ]
        ...
        for results in scenario_results_list:
            ...
            for criterion in comparison_criteria:
                if criterion in results.statistical_summary:
                    scenario_metrics[scenario_id][criterion] = (
                        results.statistical_summary[criterion]["mean"]
                    )
        ...
        for criterion in comparison_criteria:
            ...
            if criterion_values:
                analysis[criterion] = {
                    ...
                    "coefficient_of_variation": statistics.stdev(criterion_values)
                    / statistics.mean(criterion_values)
                    if statistics.mean(criterion_values) != 0
                    and len(criterion_values) > 1
                    else 0.0,
                }
    ...
    def _rank_scenarios(
        ...
        for criterion in comparison_criteria:
            ...
            ranking_analysis[criterion] = [
                scenario_id for scenario_id, _ in scenario_scores
            ]
        ...
        for i, criterion1 in enumerate(comparison_criteria):
            for j, criterion2 in enumerate(comparison_criteria):
                if i < j:
                    ...
                    for results in scenario_results_list:
                        if (
                            criterion1 in results.statistical_summary
                            and criterion2 in results.statistical_summary
                        ):
                    ...
                    if trade_off_ratios:
                        trade_off_analysis[trade_off_key] = {
                            ...
                            "ratio_variance": statistics.variance(trade_off_ratios)
                            if len(trade_off_ratios) > 1
                            else 0.0,
                        }
                ...
                if criterion in scenario_raw_scores[scenario_id]:
                    ...
                    if criterion_max > criterion_min:
                        ...
                        if criterion in negative_metrics:
                            normalized_score = 1.0 - (raw_score - criterion_min) / (
                                criterion_max - criterion_min
                            )
                        else:
                            normalized_score = (raw_score - criterion_min) / (
                                criterion_max - criterion_min
                            )
                    else:
    ...
    async def get_scenario_modeling_metrics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "most_used_scenario_types": [
                scenario_type
                for scenario_type, count in Counter(scenario_types).most_common(3)
            ],
            "most_used_simulation_methods": [
                method for method, count in Counter(simulation_methods).most_common(3)
            ],
