...
class CapacityAnalysis:
    ...
    time_to_capacity: timedelta | None = None
...
class UsageForecaster:
    ...
    def _initialize_forecasting_models(self) -> None:
        ...
        self.forecasting_models = {
            ...
            ResourceType.STORAGE_USAGE: {
                ...
                "secondary": ModelType.GRADIENT_BOOSTING,
                "seasonal": ModelType.SEASONAL_ARIMA,
            },
            ResourceType.AUTOMATION_EXECUTIONS: {
                "primary": ModelType.SEASONAL_ARIMA,
                "secondary": ModelType.ARIMA,
                "seasonal": ModelType.SEASONAL_ARIMA,
            },
        }
    ...
    async def generate_forecast(
        ...
        try:
            ...
            if (
                resource_type not in self.resource_data
                or not self.resource_data[resource_type]
            ):
                return Either.left(
                    ForecastingError(
            ...
            forecast = ResourceForecast(
                ...
                capacity_recommendations=capacity_analysis.recommended_actions
            ...
            # Cache forecast
            cache_key = (
                f"{resource_type.value}_{forecast_period_days}_{granularity.value}"
            )
    ...
    def _get_latest_consolidated_data(
        ...
        # Sort by timestamp
        combined_data = list(zip(all_timestamps, all_values, strict=False))
        combined_data.sort(key=lambda x: x[0])

        timestamps, values = (
            zip(*combined_data, strict=False) if combined_data else ([], [])
        )

        return TimeSeriesData(timestamps=list(timestamps), values=list(values))
    ...
    def _detect_seasonality(self, data: TimeSeriesData) -> tuple[bool, list[str]]:
        ...
        for timestamp, value in zip(data.timestamps, data.values, strict=False):
    ...
    def _calculate_correlation(self, list1: list[float], list2: list[float]) -> float:
        ...
        numerator = sum(
            (x - mean1) * (y - mean2) for x, y in zip(list1, list2, strict=False)
        )
    ...
    def _calculate_data_quality(self, data: TimeSeriesData) -> float:
        ...
        valid_points = sum(
            1 for v in data.values if v is not None and isinstance(v, int | float)
        )
        ...
        if len(data.timestamps) > 1:
            ...
            if time_diffs:
                ...
                consistency_score = (
                    1.0 - min(1.0, std_diff / mean_diff) if mean_diff > 0 else 0.0
                )
            else:
    ...
    def _seasonal_arima_forecast(
        ...
        for i in range(forecast_points):
            ...
            seasonal_factor = (
                seasonal_components[seasonal_index]
                / statistics.mean(seasonal_components)
    ...
    def _apply_scenario_adjustments(
        ...
        for _i, value in enumerate(base_forecast):
            ...
            for _factor, multiplier in scenario.external_factors.items():
    ...
    def _extract_seasonality_patterns(self, data: TimeSeriesData) -> dict[str, Any]:
        ...
        if len(weekly_stats) >= 5:
            weekly_averages = {
                day: statistics.mean(values) for day, values in weekly_stats.items()
            }
        ...
        if len(hourly_stats) >= 12:
            hourly_averages = {
                hour: statistics.mean(values) for hour, values in hourly_stats.items()
            }
    ...
    async def get_forecasting_summary(self) -> dict[str, Any]:
        ...
        try:
            ...
            total_data_points = sum(
                len(data.values)
                for data_list in self.resource_data.values()
