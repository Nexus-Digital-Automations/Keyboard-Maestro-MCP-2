...
class APIVersionManager:
    ...
    def register_migration(
        ...
        try:
            ...
            from_exists = any(
                v.version_string == migration.from_version
                for v in self.versions.values()
            )
            to_exists = any(
                v.version_string == migration.to_version for v in self.versions.values()
            )
    ...
    async def resolve_version(
        ...
        try:
            ...
            available_versions = [
                self.versions[vid] for vid in self.service_versions[service_id]
            ]
            ...
            self.metrics["average_resolution_time_ms"] = (
                current_avg * (total_resolutions - 1) + resolution_time
            ) / total_resolutions
    ...
    async def _find_best_version_match(
        ...
        stable_versions = [
            v for v in available_versions if v.status == VersionStatus.STABLE
        ]
        if stable_versions:
    ...
    def _find_migration(
        ...
        for migration in self.migrations.values():
            if (
                ...
                and migration.service_id == service_id
            ):
                return migration
        return None
    ...
    def _validate_version_format(self, version_string: str) -> bool:
        ...
        return bool(re.match(sequential_pattern, version_string))
    ...
    def _get_cached_resolution(self, cache_key: str) -> VersioningDecision | None:
        ...
        if (
            not cache_time
            or (datetime.now(UTC) - cache_time).total_seconds() > self.cache_ttl_seconds
        ):
            # Cache expired
    ...
    def _cache_resolution(
        ...
        if len(self.resolution_cache) > 1000:
            ...
            oldest_keys = sorted(self.cache_timestamps.items(), key=lambda x: x[1])[
                :100
            ]
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        return {
            **self.metrics,
            ...
            / max(1, self.metrics["total_resolutions"]),
