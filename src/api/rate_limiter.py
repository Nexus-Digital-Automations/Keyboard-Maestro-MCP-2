...
class RateLimitRule:
    ...
    token_bucket_size: int | None = None

    # Adaptive limiting
    adaptive_factor: float = 1.0  # Adjustment factor for adaptive limits
    min_limit: int | None = None  # Minimum limit for adaptive
    max_limit: int | None = None  # Maximum limit for adaptive
...
class QuotaDefinition:
    ...
    reset_schedule: str | None = None  # Cron-like schedule
...
class AdvancedRateLimiter:
    ...
    def remove_rule(self, rule_id: str) -> Either[APIOrchestrationError, bool]:
        ...
        try:
            if rule_id in self.rules:
                ...
                states_to_remove = [
                    k for k in self.states if k.startswith(f"{rule_id}:")
                ]
    ...
    async def check_rate_limit(
        ...
        try:
            ...
            self.performance_metrics["average_check_time_ms"] = (
                current_avg * (total_checks - 1) + check_time
            ) / total_checks
    ...
    async def _check_token_bucket(
        ...
        refill_rate = rule.token_refill_rate or (
            rule.limit / self._get_window_duration(rule.window).total_seconds()
        )
    ...
    def _check_burst_limit(
        ...
        if (
            not state.burst_window_start
            or (now - state.burst_window_start).total_seconds()
        ...
        while True:
            try:
                ...
                for key, state in self.states.items():
                    ...
                    if (
                        state.last_request
                        and (now - state.last_request).total_seconds() > 86400
                    ):
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        return {
            **self.performance_metrics,
            ...
            "success_rate": (total_requests - total_rejected) / total_requests
            if total_requests > 0
            else 1.0,
        }

    def get_rule_metrics(self, rule_id: str) -> dict[str, Any] | None:
