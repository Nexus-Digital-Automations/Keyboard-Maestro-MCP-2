...
class MetricSeries:
    ...
    def _update_aggregations(self) -> str:
        ...
        recent_values = [
            point.value for point in list(self.data_points)[-100:]
        ]  # Last 100 points
    ...
    def _calculate_rate(self) -> float:
        ...
        time_span = (
            recent_points[-1].timestamp - recent_points[0].timestamp
        ).total_seconds()
        if time_span <= 0:
            return 0.0
...
class AnomalyDetector:
    ...
    async def _check_point_anomaly(
        ...
        if "error" in point.metric_name.lower():
            ...
            "throughput" in point.metric_name.lower()
            or "rps" in point.metric_name.lower()
        ):
...
class RealTimeMonitor:
    ...
    async def collect_metric(
        ...
        try:
            ...
            self.monitor_metrics["average_collection_time_ms"] = (
    ...
    async def _alert_evaluation_loop(self) -> Any:
        ...
        while True:
            try:
                ...
                self.monitor_metrics["average_alert_evaluation_time_ms"] = (
                    current_avg + evaluation_time
                ) / 2
    ...
    async def _send_notifications(
        ...
        if (
            ...
            and (datetime.now(UTC) - alert.last_notification).total_seconds()
            < rule.notification_interval_minutes * 60
        ):
            return
    ...
    async def _broadcast_metric_update(self, metric: MetricPoint) -> None:
        ...
        message = {"type": "metric_update", "data": metric.to_dict()}
    ...
    async def _broadcast_anomaly_detection(self, anomaly: dict[str, Any]) -> Any:
        ...
        message = {"type": "anomaly_detected", "data": anomaly}
