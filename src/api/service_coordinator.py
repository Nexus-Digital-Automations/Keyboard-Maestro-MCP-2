...
class ServiceRegistry:
    ...
    def get_healthy_services(self) -> list[ServiceId]:
        ...
        return [
            service_id
            for service_id, health in self.health_status.items()
...
class ServiceCoordinator:
    ...
    async def _execute_scatter_gather(
        ...
        for step, task in scatter_tasks:
            try:
                ...
                step_duration = (
                    scatter_end_time - scatter_start_time
                ).total_seconds() * 1000
    ...
    def _create_orchestration_result(
        ...
        performance_metrics = {
            ...
            "average_step_duration": sum(r.get("duration_ms", 0) for r in step_results)
            / len(step_results)
            if step_results
            else 0,
