...
from ..core.types import WorkflowID
...
class WorkflowEngine:
    ...
    @require(
        lambda definition: definition.name and definition.steps,
        "Definition must have name and steps",
    )
    @ensure(
        ...
        while retry_count <= max_retries:
            ...
            if retry_count <= max_retries and result.state == WorkflowState.FAILED:
                delay = (
                    step.retry_config.get(
                        "delay_seconds",
                        execution.workflow_definition.retry_delay_seconds,
                    )
                    if step.retry_config
                    else execution.workflow_definition.retry_delay_seconds
                )
    ...
    async def _execute_parallel_step(
        ...
        tasks = []
        for _task_config in parallel_tasks:
    ...
    def _substitute_template_variables(
        ...
        if isinstance(data, dict):
            return {
                key: self._substitute_template_variables(value, context)
                if isinstance(value, dict | list)
                else str(value).format(**context)
