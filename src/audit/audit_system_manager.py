...
class AuditSystemManager:
    ...
    def __init__(self, config: AuditConfiguration | None = None):
    ...
    async def audit_tool_execution(
        ...
        try:
            ...
            if len(self.performance_metrics["audit_latency"]) > 1000:
                self.performance_metrics["audit_latency"] = self.performance_metrics[
                    "audit_latency"
                ][-1000:]
    ...
    def get_system_status(self) -> dict[str, Any]:
        ...
        try:
            ...
            if time_delta > 0:
                self.performance_metrics["events_per_second"] = logger_stats[
                    "events_logged"
                ] / max(logger_stats["uptime_seconds"], 1)
                self.performance_metrics["compliance_checks_per_second"] = (
                    monitor_stats["rules_evaluated"]
                    / max(monitor_stats["uptime_seconds"], 1)
                )
            ...
            return {
                ...
                "performance": {
                    ...
                    "compliance_checks_per_second": self.performance_metrics[
                        "compliance_checks_per_second"
                    ],
                },
                "background_services": {
                    "active_tasks": len(
    ...
    def _extract_compliance_tags(
        ...
        if "file_operations" in tool_name:
            ...
            param_text = str(parameters).lower()
            if any(
                keyword in param_text
                for keyword in ["patient", "medical", "health", "phi"]
            ):
                tags.add("phi_access")
        ...
        try:
            while True:
                ...
                for cache_key, report in self.report_generator.report_cache.items():
                    if (
                        current_time - report.generated_at
                        > self.report_generator.cache_expiry
                    ):
...
_audit_system: AuditSystemManager | None = None


def get_audit_system() -> AuditSystemManager | None:
...
async def initialize_audit_system(
    ...
    compliance_standards: list[ComplianceStandard] | None = None,
) -> Either[AuditError, AuditSystemManager]:
