        ...
        try:
            ...
            encrypted_data = bytes(
                ...
                for a, b in zip(
                    data_bytes,
                    self.encryption_key
                    * (len(data_bytes) // len(self.encryption_key) + 1),
                    strict=False,
                )
            )
        ...
        try:
            ...
            decrypted_data = bytes(
                ...
                for a, b in zip(
                    encrypted_payload,
                    self.encryption_key
                    * (len(encrypted_payload) // len(self.encryption_key) + 1),
                    strict=False,
                )
            )
...
class EventLogger:
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "events_per_second": self.stats["events_logged"]
            / max(uptime.total_seconds(), 1),
            "success_rate": (
                self.stats["events_logged"]
                / max(self.stats["events_logged"] + self.stats["events_failed"], 1)
            )
            * 100,
        }
