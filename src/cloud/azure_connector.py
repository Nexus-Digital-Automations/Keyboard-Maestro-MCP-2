...
class AzureConnector:
    ...
    async def connect(self, credentials: CloudCredentials) -> Either[CloudError, str]:
        ...
        try:
            ...
            session_id = (
                f"azure_{int(datetime.now(UTC).timestamp())}_{secrets.token_hex(8)}"
            )
    ...
    async def create_storage_account(
        ...
        try:
            ...
            operation.result(timeout=300)  # 5 minutes
    ...
    def _validate_storage_account_name(self, name: str) -> bool:
        ...
        return not (not name.islower() or not name.isalnum())
    ...
    async def sync_to_blob_storage(
        ...
        try:
            ...
            if source.is_file():
                ...
                for file_path in source.rglob("*"):
                    if file_path.is_file():
                        ...
                        blob_name = (
                            f"{destination_prefix}{relative_path}"
            ...
            sync_result = {
                ...
                "errors": errors,
                "success_rate": len(uploaded_files)
                / (len(uploaded_files) + len(errors))
    ...
    async def get_session_info(
        ...
        session_info = {
            ...
            "available_services": [
                ...
                AzureServiceType.VIRTUAL_MACHINES,
                AzureServiceType.SQL_DATABASE,
                AzureServiceType.FUNCTIONS,
                AzureServiceType.SERVICE_BUS,
                AzureServiceType.COGNITIVE_SERVICES,
            ],
        }
    ...
    async def disconnect(self, session_id: str) -> Either[CloudError, None]:
        ...
        cache_keys_to_remove = [
            key for key in self.connection_cache if session_id in key
        ]
