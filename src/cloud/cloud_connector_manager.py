...
_cloud_manager: CloudConnectorManager | None = None
...
class CloudConnectorManager:
    ...
    async def establish_cloud_connection(
        ...
        try:
            ...
            if (
                len(self.connection_pool.get(provider, []))
    ...
    def _track_operation_metric(self, metric_name: str, value: float) -> dict[str, Any]:
        ...
        if len(self.operation_metrics[metric_name]) > 100:
            self.operation_metrics[metric_name] = self.operation_metrics[metric_name][
                -100:
            ]
...
def get_cloud_manager() -> CloudConnectorManager | None:
