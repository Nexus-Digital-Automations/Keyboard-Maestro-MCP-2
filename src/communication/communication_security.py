...
class SecurityThreat:
    ...
    def get_risk_score(self) -> float:
        ...
        severity_scores = {"low": 1.0, "medium": 3.0, "high": 7.0, "critical": 10.0}
...
class RateLimitTracker:
    ...
    def _check_sender_limits(
        ...
        email_count = sum(
            1 for _, msg_type in history if msg_type == CommunicationType.EMAIL
        )
        sms_count = sum(
            1
            for _, msg_type in history
            if msg_type in [CommunicationType.SMS, CommunicationType.IMESSAGE]
        )

        # Check limits
        if (
            comm_type == CommunicationType.EMAIL
            and email_count >= self.config.max_emails_per_hour
        ):
            return Either.left(
                RateLimitError(
        ...
        if (
            comm_type in [CommunicationType.SMS, CommunicationType.IMESSAGE]
            and sms_count >= self.config.max_sms_per_hour
        ):
            return Either.left(
                RateLimitError(
...
class SpamDetector:
    ...
    def _analyze_urls(self, text: str) -> tuple[float, list[SecurityThreat]]:
        ...
        for url in urls:
            ...
            if any(
                shortener in url
                for shortener in ["bit.ly", "tinyurl", "goo.gl", "t.co"]
            ):
...
class CommunicationSecurityManager:
    ...
    def validate_communication_security(
        ...
        try:
            ...
            if (
                request.communication_type == CommunicationType.EMAIL
    ...
    def _validate_attachments(
        ...
        for attachment_path in attachments:
            ...
            extension = (
                attachment_path.lower().split(".")[-1] if "." in attachment_path else ""
            )
