...
class EmailSecurityValidator:
    ...
    def _contains_malicious_content(text: str) -> bool:
        ...
        text_lower = text.lower()
        return any(
            re.search(pattern, text_lower, re.IGNORECASE | re.DOTALL)
    ...
    def _is_likely_spam(subject: str, body: str) -> bool:
        ...
        matches = sum(
            1 for pattern in spam_indicators if re.search(pattern, combined_text)
        )
...
class EmailManager:
    ...
    def _build_email_applescript(
        ...
        try:
            ...
            # Complete script
            script_parts.extend(["", "        send", "    end tell", "end tell"])
    ...
    async def get_mail_accounts(
        self,
    ) -> Either[CommunicationError, list[dict[str, str]]]:
