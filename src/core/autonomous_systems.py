...
class LearningExperience:
    ...
    def extract_patterns(self) -> dict[str, Any]:
        ...
        patterns["performance_correlations"] = {
            "confidence_vs_success": self.action_taken.confidence
            if self.success
            else -self.action_taken.confidence,
            "impact_vs_performance": self.performance_impact,
            "cost_efficiency": self.performance_impact
            / max(self.action_taken.estimate_total_cost(), 0.1),
        }
...
class AgentConfiguration:
    ...
    def should_request_human_approval(self, action: AgentAction) -> bool:
        ...
        return self.autonomy_level == AutonomyLevel.MANUAL
...
class AutonomousAgentError(ValidationError):
    ...
    def action_too_risky(
        ...
        return cls(
            "risk_score",
            risk_score,
            f"Action risk {risk_score} exceeds limit {max_risk}",
        )
    ...
    def resource_limit_exceeded(
        ...
        return cls(
            "resource_usage",
            usage,
            f"Resource {resource} usage {usage} exceeds limit {limit}",
        )
