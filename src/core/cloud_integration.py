...
class CloudError:
    ...
    def insecure_auth_method(cls, method: CloudAuthMethod) -> CloudError:
        return cls(
            "INSECURE_AUTH_METHOD",
            f"Authentication method {method.value} not allowed for enterprise",
        )
    ...
    def unsupported_provider_for_operation(cls, provider: CloudProvider) -> CloudError:
        return cls(
            "UNSUPPORTED_PROVIDER_FOR_OPERATION",
            f"Provider {provider.value} does not support this operation",
        )
    ...
    def connection_establishment_failed(cls, details: str) -> CloudError:
        return cls(
            ...
            f"Failed to establish connection: {details}",
        )

    @classmethod
    def sync_not_supported_for_provider(cls, provider: CloudProvider) -> CloudError:
        return cls(
            "SYNC_NOT_SUPPORTED_FOR_PROVIDER",
            f"Sync not supported for {provider.value}",
        )
