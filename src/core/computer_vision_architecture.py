...
def create_image_content(image_data: bytes) -> ImageContent:
    ...
    is_valid_image = any(image_data.startswith(sig) for sig in image_signatures)
...
class VisionError(Exception):
    ...
    message: str
    error_code: str
    operation: VisionOperation | None = None
...
class VisionProcessingRequest:
    ...
    roi_coordinates: tuple[float, float, float, float] | None = (
...
def non_maximum_suppression(
    ...
    for obj in sorted_objects:
        ...
        for kept_obj in keep:
            if (
                obj.category == kept_obj.category
                and calculate_iou(obj.bounding_box, kept_obj.bounding_box)
                > iou_threshold
            ):
                should_keep = False
                break
