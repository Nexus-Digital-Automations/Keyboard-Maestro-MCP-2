...
class BottleneckAnalysis:
    ...
    def __post_init__(self):
        require(
            lambda: len(self.impact_description.strip()) > 0,
            "Impact description required",
        )
        require(
            lambda: self.normal_range[0] <= self.normal_range[1],
            "Normal range must be valid",
        )
...
class OptimizationRecommendation:
    ...
    def __post_init__(self):
        ...
        require(
            lambda: 0 <= self.expected_improvement <= 100,
            "Expected improvement must be 0-100%",
        )
...
class PerformanceMonitoringError(Exception):
    ...
    def __init__(self, message: str, session_id: MonitoringSessionID | None = None):
