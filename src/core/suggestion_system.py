...
class SuggestionError(ValidationError):
    ...
    def initialization_failed(cls, message: str) -> SuggestionError:
        return cls(
            "SUGGESTION_INIT_FAILED",
            f"Suggestion system initialization failed: {message}",
        )
...
class AutomationPerformanceMetrics:
    ...
    def needs_optimization(self, threshold: float = 0.7) -> bool:
        ...
        return (
            self.get_performance_score() < threshold
            or self.trend_direction == "declining"
        )
...
class SuggestionContext:
    ...
    user_id: str
    current_automation: str | None = None
...
class SuggestionFeedback:
    ...
    def __post_init__(self):
        ...
        if self.effectiveness_score is not None and not (
            0.0 <= self.effectiveness_score <= 1.0
        ):
...
class SuggestionSecurityValidator:
    ...
    def _contains_sensitive_data(self, data: dict[str, Any]) -> bool:
        ...
        try:
            ...
            return any(
                re.search(pattern, data_str) for pattern in self.SENSITIVE_PATTERNS
            )
        except Exception:
