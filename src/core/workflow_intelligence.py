...
class IntelligentWorkflowTemplate:
    ...
    generated_from_pattern: PatternId | None
    ...
    def __post_init__(self):
        ...
        if not (0.0 <= self.pattern_discovery_threshold <= 1.0):
            raise ValidationError(
                ...
                self.pattern_discovery_threshold,
                "must be between 0.0 and 1.0",
            )
...
def validate_workflow_component(component: WorkflowComponent) -> bool:
    ...
    return component.component_type in ["action", "condition", "trigger", "group"]
...
def calculate_workflow_complexity_score(components: list[WorkflowComponent]) -> float:
    ...
    dependency_factor = (
        sum(len(c.dependencies) for c in components) / len(components) / 5.0
    )  # Normalize

    # Weighted combination
    overall_complexity = (
        component_count_factor * 0.3
        + avg_complexity * 0.5
