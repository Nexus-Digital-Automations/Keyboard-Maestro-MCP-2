...
def calculate_composite_trust_score(
    ...
    if weights is None:
        weights = {"identity": 0.3, "device": 0.25, "behavior": 0.25, "location": 0.2}
...
def validate_security_context(
    context: SecurityContext,
) -> Either[ZeroTrustError, SecurityContext]:
...
def generate_compliance_recommendations(assessment: ComplianceAssessment) -> list[str]:
...
def validate_trust_result(validation_result: TrustValidationResult) -> bool:
    ...
    return all(
        criteria in validation_result.criteria_results for criteria in required_criteria
    )
