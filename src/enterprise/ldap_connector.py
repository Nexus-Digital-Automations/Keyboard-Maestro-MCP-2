...
class LDAPConnector:
    ...
    async def connect(
        ...
        try:
            ...
            if not conn.bind():
                getattr(conn, "last_error", "Unknown authentication error")
    ...
    @require(
        lambda _self, search_base: isinstance(search_base, str)
        and len(search_base) > 0,
    )
    async def search_users(
        ...
        size_limit: int = 1000,
    ) -> Either[EnterpriseError, list[LDAPUser]]:
    ...
    def _convert_ldap_entry_to_user(
        self,
        entry: list[Any] | str,
    ) -> Either[EnterpriseError, LDAPUser]:
    ...
    async def _process_user_sync(
        ...
        sync_options.get("update_existing", True)
        sync_options.get("create_missing", True)
