...
class SSOManager:
    ...
    async def handle_sso_callback(
        ...
        try:
            ...
            if (
                "state" in callback_data
                and callback_data["state"] != request_info["state"]
            ):
                return Either.left(
                    EnterpriseError(
                        "INVALID_STATE",
    ...
    def get_provider_status(self, provider_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "last_auth": provider["last_auth"].isoformat()
            if provider["last_auth"]
            else None,
    ...
    def _get_certificate_expiry(self, certificate: str) -> str | None:
