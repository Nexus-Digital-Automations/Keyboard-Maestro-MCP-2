...
from ..core.user_identity_architecture import (
    UserProfile, UserSession, IdentityError, SecurityLevel,
    UserSessionId, UserProfileId, SessionToken,
    generate_session_id, generate_session_token
)
...
class SessionSwitchRequest:
    ...
    current_session_id: Optional[UserSessionId]
...
class SessionManager:
    ...
    async def create_session(
        ...
        try:
            ...
            session_limit = self.concurrent_session_limits.get(user_profile_id, 5)  # Default limit
    ...
    async def get_session(
        ...
        try:
            ...
            return Either.error(IdentityError(
                f"Session retrieval error: {str(e)}",
    ...
    async def switch_user_context(
        ...
        try:
            ...
            if switch_request.security_validation:
                security_result = await self._validate_context_switch_security(switch_request)
            ...
            if switch_request.preserve_context and current_session_id:
                current_context_result = await self.get_session_context(current_session_id)
                if current_context_result.is_success():
                    preserved_context = current_context_result.value.temporary_data.copy()
            ...
            if target_sessions:
                ...
                if preserved_context:
                    await self._merge_session_context(target_session_id, preserved_context)
                ...
                if context_result.is_success():
                    ...
                    logger.info(f"Switched to existing session {target_session_id} for user {target_user_id}")
    ...
    async def _validate_context_switch_security(
        ...
        try:
            ...
            if current_session_id:
                ...
                if ("admin" not in current_permissions and 
                    "user_management" not in current_permissions):
                    return Either.error(IdentityError(
    ...
    async def terminate_session(
        ...
        try:
            ...
            async with self.session_locks[session_id]:
                ...
                logger.info(f"Terminated session {session_id} for user {user_profile_id}, reason: {reason}")
                return Either.success(True)
            
        except Exception as e:
            logger.error(f"Session termination failed: {e}")
            return Either.error(IdentityError(
                f"Session termination error: {str(e)}",
    ...
    async def get_user_sessions(
        ...
        try:
            ...
            for session_id in user_session_ids:
                if session_id in self.active_sessions:
                    ...
                    session_info = {
                        ...
                        "isolation_level": context.isolation_level if context else "unknown"
                    }
            ...
            return Either.error(IdentityError(
                f"User sessions retrieval error: {str(e)}",
                "USER_SESSIONS_ERROR"
            ))
    ...
    async def get_session_analytics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for session in self.active_sessions.values():
                duration = (current_time - session.created_at).total_seconds() / 60  # minutes
            ...
            recent_transitions = [
                t for t in self.session_transitions 
    ...
    async def set_concurrent_session_limit(
        ...
        try:
            ...
            return Either.error(IdentityError(
                f"Session limit setting error: {str(e)}",
                "LIMIT_SETTING_ERROR"
            ))