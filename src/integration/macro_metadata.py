...
from ..core.types import Duration, MacroId
...
class UsageStatistics:
    ...
    average_execution_time: Duration | None = None
...
class MacroMetadataExtractor:
    ...
    async def extract_enhanced_metadata(
        ...
        optimization_suggestions = self._generate_optimization_suggestions(
            ...
            actions,
            complexity,
            usage_stats.get_right() if usage_stats.is_right() else UsageStatistics(),
        )
        ...
        enhanced_metadata = EnhancedMacroMetadata(
            ...
            usage_stats=usage_stats.get_right()
            if usage_stats.is_right()
            else UsageStatistics(),
            relationships=relationships.get_right()
            if relationships.is_right()
            else MacroRelationships(),
    ...
    def _analyze_actions(self, action_data: list[dict[str, Any]]) -> list[ActionInfo]:
        ...
        for action in action_data:
            ...
            if any(
                keyword in action_type for keyword in ["type", "text", "paste", "copy"]
            ):
                category = ActionCategory.TEXT_MANIPULATION
            elif any(
                keyword in action_type for keyword in ["application", "app", "window"]
            ):
                category = ActionCategory.APPLICATION_CONTROL
            elif any(
                keyword in action_type
                for keyword in ["system", "volume", "brightness", "sleep"]
            ):
                category = ActionCategory.SYSTEM_CONTROL
            elif any(
                keyword in action_type for keyword in ["file", "folder", "path", "save"]
            ):
                category = ActionCategory.FILE_OPERATIONS
            elif any(
                keyword in action_type
                for keyword in ["if", "while", "repeat", "pause", "stop"]
            ):
                category = ActionCategory.WORKFLOW_CONTROL
            elif any(
                keyword in action_type for keyword in ["script", "shell", "applescript"]
            ):
                category = ActionCategory.CUSTOM_SCRIPT
            elif any(
                keyword in action_type
                for keyword in ["click", "move", "display", "notification"]
            ):
    ...
    def _assess_complexity(
        ...
        script_actions = sum(
            1 for a in actions if a.category == ActionCategory.CUSTOM_SCRIPT
        )
        workflow_actions = sum(
            1 for a in actions if a.category == ActionCategory.WORKFLOW_CONTROL
        )
    ...
    def _generate_optimization_suggestions(
        ...
        if usage_stats.total_executions == 0:
            ...
            usage_stats.last_30_days_executions == 0
            and usage_stats.total_executions < 5
        ):
    ...
    def _parse_hotkey(self, hotkey_data: dict[str, Any]) -> str | None:
        ...
        # Build hotkey string
        modifier_symbols = {"Command": "⌘", "Option": "⌥", "Shift": "⇧", "Control": "⌃"}
    ...
    def _get_cached_metadata(self, macro_id: MacroId) -> EnhancedMacroMetadata | None:
