...
class SmartMacroFilter:
    ...
    def search_macros(
        ...
        if query.action_categories:
            filtered_macros = [
                ...
                if m.primary_function in query.action_categories
            ]
        ...
        if query.complexity_levels:
            filtered_macros = [
                m for m in filtered_macros if m.complexity in query.complexity_levels
            ]
        ...
        if query.max_days_since_used is not None:
            ...
            filtered_macros = [
                m
                for m in filtered_macros
                if m.usage_stats.last_executed
                and m.usage_stats.last_executed >= cutoff_date
            ]
    ...
    def generate_insights(self, macros: list[EnhancedMacroMetadata]) -> dict[str, Any]:
        ...
        low_success_macros = [
            m
            for m in macros
            if m.usage_stats.success_rate < 0.8 and m.usage_stats.total_executions > 5
        ]
        ...
        for macro in macros:
            ...
            if scope == SearchScope.NAME_ONLY:
                ...
                if (
                    ...
                    or search_lower in macro.group.lower()
                ):
                    match_found = True
                ...
                if any(
                    search_lower in content.lower() for content in searchable_metadata
                ):
                    match_found = True
