...
class LearningEngine:
    ...
    def _extract_temporal_features(
        ...
        features = {
            ...
            "completion_time_variance": statistics.stdev(completion_times)
    ...
    def _extract_sequence_features(
        ...
        unique_sequences = len({tuple(seq) for seq in all_sequences})
        ...
        features = {
            ...
            "average_sequence_length": statistics.mean(sequence_lengths)
            if sequence_lengths
            else 0.0,
            "sequence_length_variance": statistics.stdev(sequence_lengths)
    ...
    def _extract_tool_usage_features(
        ...
        features = {
            ...
            "tool_diversity_score": len(tool_usage) / total_usage
            if total_usage > 0
            else 0.0,
            "primary_tool_usage_ratio": most_used_tool_usage / total_usage
            if total_usage > 0
            else 0.0,
    ...
    def _extract_performance_features(
        ...
        features = {
            ...
            "efficiency_variance": statistics.stdev(efficiency_scores)
            if len(efficiency_scores) > 1
            else 0.0,
            "average_reliability": statistics.mean(reliability_scores)
            if reliability_scores
            else 0.0,
            "reliability_variance": statistics.stdev(reliability_scores)
            if len(reliability_scores) > 1
            else 0.0,
            "average_success_rate": statistics.mean(success_rates)
            if success_rates
            else 0.0,
            "success_rate_variance": statistics.stdev(success_rates)
    ...
    def _extract_context_features(
        ...
        features = {
            ...
            "context_diversity": len(all_context_tags) / len(patterns)
            if patterns
            else 0.0,
    ...
    def _calculate_sequence_complexity(self, sequences: list[list[str]]) -> float:
        ...
        for sequence in sequences:
            ...
            complexity = (
                len(sequence) * (unique_actions / len(sequence)) if sequence else 0
            )
    ...
    def _find_common_subsequences(
        ...
        common_subsequences = [
            subseq for subseq, count in subsequence_counts.items() if count >= 2
        ]
