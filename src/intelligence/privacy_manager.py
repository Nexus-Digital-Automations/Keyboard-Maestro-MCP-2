...
class PrivacyManager:
    ...
    async def _anonymize_result_data(
        ...
        for key, value in results.items():
            if isinstance(value, str):
                ...
                anonymized[key] = [
                    await self._anonymize_result_data(item, privacy_level)
                    if isinstance(item, dict)
                    else self._anonymize_string_value(str(item), privacy_level)
    ...
    def _log_data_access(
        ...
        access_entry = {
            ...
            - (len(filtered_results) / max(1, len(original_results))),
        }
