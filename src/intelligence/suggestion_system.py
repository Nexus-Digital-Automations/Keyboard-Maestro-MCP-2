...
class AutomationSuggestion:
    ...
    def get_roi_estimate(self) -> float:
        ...
        roi = (
            self.potential_time_saved * effective_success_rate * effective_confidence
        ) / implementation_cost
        return roi
...
class IntelligentSuggestionSystem:
    ...
    async def generate_suggestions(
        ...
        try:
            ...
            error_prevention_suggestions = (
                await self._generate_error_prevention_suggestions(patterns)
            )
    ...
    async def _generate_automation_suggestions(
        ...
        automation_candidates = [
            ...
            if (
                pattern.frequency >= 5
                and pattern.confidence_score >= 0.8
                and pattern.success_rate >= 0.8
                and len(pattern.action_sequence) >= 2
            )
        ]
        ...
        for pattern in automation_candidates[: self.max_suggestions_per_category]:
            ...
            time_saved_per_occurrence = (
                pattern.average_completion_time * 0.8
            ...
            if total_potential_savings >= self.min_time_savings_threshold:
                suggestion = AutomationSuggestion(
                    ...
                    * 0.9,  # Slightly lower for automation
        ...
        for tool, usage_data in tool_usage.items():
            ...
            if usage_data["frequency"] >= 5 and avg_efficiency < 0.8:
                suggestion = AutomationSuggestion(
                    ...
                    confidence=0.7,
                    potential_time_saved=usage_data["frequency"]
                    * 30.0,  # Estimate 30s savings per use
                    implementation_complexity="low",
                    tools_involved=[tool],
                    trigger_conditions={
                        "tool": tool,
                        "efficiency_below": avg_efficiency,
                    },
                    estimated_success_rate=0.75,
                    rationale=f"Tool used {usage_data['frequency']} times with {avg_efficiency:.1%} average efficiency",
                    supporting_patterns=[
                        p.pattern_id for p in usage_data["patterns"][:3]
                    ],
                )
                suggestions.append(suggestion)
    ...
    async def _generate_error_prevention_suggestions(
        ...
        for pattern in error_prone_patterns[: self.max_suggestions_per_category]:
            ...
            potential_error_reduction = (
                error_rate * 0.7
            )  # Assume 70% error reduction possible
