...
class SecurityManager:
                    ...
                    if threat_confidence > 0.8 and device_id:
                        await self._log_security_event(
                            ...
                            details={
                                ...
                                "threat_name": signature.threat_name,
                                "confidence": threat_confidence,
                                "data_sample": communication_data[
                                    :100
                                ],  # First 100 chars
                            },
                        )
    ...
    async def _validate_credential_data(
        ...
        if auth_method == AuthenticationMethod.CERTIFICATE:
            if (
                ...
                or "private_key" not in credential_data
            ):
                return Either.error(
                    IoTIntegrationError(
    ...
    async def _handle_auth_failure(
        ...
        self.auth_failure_counts[device_id] = (
            self.auth_failure_counts.get(device_id, 0) + 1
        )
    ...
    async def get_security_summary(self) -> dict[str, Any]:
        ...
        recent_events = [
            ...
            if e.detected_at > datetime.now(UTC) - timedelta(hours=24)
        ]
        ...
        return {
            ...
            "auth_success_rate": (
                self.successful_auths / max(self.total_auth_attempts, 1)
            )
            * 100,
...
def validate_certificate_format(certificate: str) -> bool:
    ...
    return (
        ...
        and certificate.endswith("-----END CERTIFICATE-----")
        and len(certificate) > 100
    )
