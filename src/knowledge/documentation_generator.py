    ...
    async def _process_macro_documentation(
        ...
        try:
            ...
            if config.include_parameters:
                content_data["sections"]["parameters"] = (
                    self._generate_macro_parameters(macro_data)
                )
            ...
            if config.include_troubleshooting:
                content_data["sections"]["troubleshooting"] = (
                    self._generate_macro_troubleshooting(macro_data)
                )
...
"""
    ...
    def _generate_markdown_content(self, content_data: dict[str, Any]) -> str:
        ...
        for _section_name, section_content in sections.items():
            if section_content:
