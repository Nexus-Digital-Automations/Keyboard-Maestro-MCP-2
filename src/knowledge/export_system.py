...
class ExportOptions:
    ...
    def get_filename(self, title: str, timestamp: datetime | None = None) -> str:
...
class MarkdownExporter:
    ...
    def __init__(self, branding: BrandingOptions | None = None):
...
class HTMLExporter:
    ...
    async def export_document(
        ...
        # Close HTML
        html_parts.extend(["</main>", self._get_footer(), "</body>", "</html>"])
...
class ExportManager:
    ...
    @require(
        lambda export_scope, target_id: export_scope and target_id,
        "Export scope and target required",
    )
    @ensure(
    ...
    async def execute_export_job(
        ...
        try:
            ...
            if not exporter_class:
                ...
                job.error_message = (
                    f"Unsupported export format: {job.options.format.value}"
                )
