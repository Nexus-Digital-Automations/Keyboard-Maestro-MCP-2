...
class SearchQuery:
    ...
    categories: set[KnowledgeCategory] | None = None
    ...
    def _create_document_vector(
        ...
        try:
            ...
            for term in terms:
                ...
                idf = (
                    math.log(len(self.documents) + 1) if len(self.documents) > 0 else 1
                )
                vector[term] = tf * idf
...
class SearchEngine:
            ...
            for result in results:
                ...
                if document:
                    ...
                    if query.boost_recent:
                        days_old = (
                            datetime.now(UTC) - document.metadata.modified_at
                        ).days
    ...
    def _generate_suggestions(self, query: SearchQuery) -> list[str]:
        ...
        try:
            ...
            for popular_query, _count in self.popular_queries.most_common(10):
            ...
            for term in query_terms:
                ...
                related_terms = set()
                for indexed_term in self.index.term_index:
    ...
    def _generate_facets(
        ...
        try:
            facets = {"categories": defaultdict(int), "tags": defaultdict(int)}
    ...
    async def get_search_analytics(self) -> dict[str, Any]:
        ...
        try:
            return {
                ...
                "query_types": Counter(
                    q.search_type.value for q in self.search_history
                ),
                "average_results": sum(
                    len(self.index.term_index.get(term, set()))
                    for term in self.index.term_index
                )
                / max(1, len(self.index.term_index)),
            }
