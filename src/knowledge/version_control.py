...
class VersionManager:
    ...
    async def get_change_history(
        ...
        try:
            ...
            if document_id:
                filtered_changes = [
                    c for c in filtered_changes if c.document_id == document_id
                ]
    ...
    async def get_version_analytics(self) -> dict[str, Any]:
        ...
        try:
            ...
            for change in self.change_history:
                change_types[change.change_type.value] = (
                    change_types.get(change.change_type.value, 0) + 1
                )

            # Author activity
            author_activity = {}
            for change in self.change_history:
                author_activity[change.author] = (
                    author_activity.get(change.author, 0) + 1
                )
