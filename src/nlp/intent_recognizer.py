    ...
    def _initialize_entity_patterns(self) -> dict[EntityType, list[re.Pattern]]:
        ...
        return {
            EntityType.APPLICATION: [
                re.compile(
                    r"\b(?:app|application|program)\s+([A-Za-z][A-Za-z0-9\s]*)",
                    re.IGNORECASE,
                ),
                re.compile(
                    r"\b(Chrome|Firefox|Safari|Finder|Terminal|TextEdit|Word|Excel|PowerPoint|Photoshop|Sketch|VS Code|Xcode)\b",
                    re.IGNORECASE,
                ),
            ...
            EntityType.FILE_PATH: [
                re.compile(
                    r"(?:file|path|folder|directory)\s*[:\-]?\s*([~/][^\s]+)",
                    re.IGNORECASE,
                ),
                re.compile(
                    r"\b([~/][^\s]+\.(?:txt|doc|pdf|xlsx|png|jpg|jpeg|gif|mp4|mov|zip|dmg))\b",
                    re.IGNORECASE,
                ),
            ...
            EntityType.URL: [
                ...
                re.compile(
                    r"\b([a-zA-Z0-9-]+\.(?:com|org|net|edu|gov|io|co)(?:/[^\s]*)?)",
                    re.IGNORECASE,
                ),
            ],
            EntityType.EMAIL: [
            ...
            EntityType.DURATION: [
                re.compile(
                    r"\b(\d+(?:\.\d+)?\s*(?:second|minute|hour|day|week|month|year)s?)\b",
                    re.IGNORECASE,
                ),
                re.compile(
            ...
            EntityType.AUTOMATION_ACTION: [
                re.compile(
                    r"\b(click|press|type|open|close|launch|quit|move|resize|minimize|maximize|copy|paste|cut|delete|save|print|search|find|replace)\b",
                    re.IGNORECASE,
                ),
                re.compile(
                    r"\b(automation|macro|script|workflow|trigger|action|command|execute|run)\b",
                    re.IGNORECASE,
                ),
            ],
            EntityType.HOTKEY: [
                re.compile(
                    r"\b((?:cmd|ctrl|alt|shift|option|command|control)(?:\s*\+\s*[a-zA-Z0-9])+)\b",
                    re.IGNORECASE,
                ),
...
class IntentClassifier:
    ...
    def _extract_intent_parameters(
        ...
        if pattern.category == IntentCategory.AUTOMATION_COMMAND:
            ...
            parameters["assistance_level"] = (
                "detailed" if "explain" in text.lower() else "quick"
            )
    ...
    def _update_learning_data(self, text: str, intents: list[RecognizedIntent]):
        ...
        for intent_name in self.learning_data:
            if len(self.learning_data[intent_name]) > 1000:
                self.learning_data[intent_name] = self.learning_data[intent_name][
                    -1000:
                ]
