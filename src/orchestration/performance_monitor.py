...
class EcosystemPerformanceMonitor:
    ...
    def _initialize_alert_thresholds(self) -> dict[str, dict[str, float]]:
        ...
        return {
            ...
            "resource_utilization": {"warning": 0.70, "error": 0.85, "critical": 0.95},
    ...
    async def get_current_metrics(self) -> SystemPerformanceMetrics:
        ...
        total_executions = sum(
            tool.execution_count for tool in self.tool_performance.values()
        )
    ...
    async def _calculate_performance_trend(self, tool_id: str) -> str:
        ...
        numerator = sum(
            (x - mean_x) * (y - mean_y)
            for x, y in zip(x_vals, recent_times, strict=False)
        )
    ...
    async def _check_performance_alerts(
        ...
        if (
            ...
            metrics.average_response_time
            > self.alert_thresholds["response_time"]["error"]
        ):
            alert = PerformanceAlert(
                timestamp=datetime.now(UTC),
        ...
        for resource, usage in metrics.resource_utilization.items():
            if usage > self.alert_thresholds["resource_utilization"]["critical"]:
                alert = PerformanceAlert(
                    ...
                    current_value=usage,
                    threshold_value=self.alert_thresholds["resource_utilization"][
                        "critical"
                    ],
    ...
    async def _generate_optimization_recommendations(
        ...
        high_usage_resources = [
            resource
            for resource, usage in current_metrics.resource_utilization.items()
            if usage > 0.8
        ]
...
_global_performance_monitor: EcosystemPerformanceMonitor | None = None
