...
from .security_sandbox import PluginSandbox, PluginSecurityManager
...
class PluginRegistry:
    ...
    def __init__(self, registry_path: Path | None = None):
        self.registry_path = (
            registry_path or Path.home() / ".km-mcp" / "plugins" / "registry.json"
        )
    ...
    def get_plugin_metadata(self, plugin_id: PluginId) -> PluginMetadata | None:
        """Get plugin metadata by ID."""
        return self.plugins.get(plugin_id)

    def get_plugin_path(self, plugin_id: PluginId) -> Path | None:
    ...
    def _deserialize_metadata(self, data: dict[str, Any]) -> PluginMetadata:
        ...
        permissions = PluginPermissions(
            permissions={
                PermissionId(p) for p in permissions_data.get("permissions", [])
            },
...
class PluginManager:
    ...
    def __init__(self, plugins_directory: Path | None = None):
        self.plugins_directory = (
            plugins_directory or Path.home() / ".km-mcp" / "plugins"
        )
    ...
    def list_plugins(self) -> list[dict[str, Any]]:
        ...
        for metadata in self.registry.list_plugins():
            plugin_info = {
                ...
                "status": "loaded"
                if metadata.identifier in self.loaded_plugins
                else "installed",
                "permissions": {
                    "network_access": metadata.permissions.network_access,
                    "file_system_access": metadata.permissions.file_system_access,
