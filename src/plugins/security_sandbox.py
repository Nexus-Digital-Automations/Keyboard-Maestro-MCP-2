    ...
    def _scan_patterns(self, content: str) -> dict[str, list[str]]:
        ...
        for pattern in self.SUSPICIOUS_PATTERNS:
            ...
            if matches:
                ...
                if any(
                    keyword in pattern
                    for keyword in ["read", "write", "delete", "remove", "mkdir"]
                ):
                    results["file_operations"].extend(matches)
                elif any(
                    keyword in pattern
                    for keyword in ["socket", "urllib", "requests", "http"]
                ):
    ...
    def _validate_permissions(
        ...
        try:
            ...
            if permissions.network_access and not scan_data["network_operations"]:
                logger.warning("Plugin requests network access but doesn't use it")

            if permissions.file_system_access and not scan_data["file_operations"]:
                logger.warning("Plugin requests file system access but doesn't use it")
