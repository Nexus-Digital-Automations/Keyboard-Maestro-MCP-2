...
class AlgorithmAnalyzer:
    ...
    ) -> Either[QuantumError, AlgorithmAnalysis]:
    ...
    async def assess_system_vulnerabilities(
        ...
        scope: str = "system",
    ) -> Either[QuantumError, VulnerabilityAssessment]:
        ...
        try:
            ...
            overall_risk_score = (
                min(1.0, total_risk_score / total_algorithms)
    ...
    async def generate_migration_roadmap(
        ...
        try:
            ...
            critical_algorithms = [
                a
                for a in prioritized_algorithms
                if a.threat_level
                in [QuantumThreatLevel.CRITICAL, QuantumThreatLevel.HIGH]
            ...
            roadmap = {
                ...
                "estimated_duration_months": sum(
                    phase["duration_months"] for phase in migration_phases
                ),
    ...
    def _initialize_algorithm_patterns(self) -> str:
        ...
        self.algorithm_patterns = {
            ...
            "des": {
                "type": "symmetric",
                "patterns": [r"\bdes\b", r"\bDES\b", r"Data.*Encryption.*Standard"],
                "default_key_size": 56,
            },
            "sha": {
                "type": "hash",
            ...
            "md5": {
                "type": "hash",
                "patterns": [r"\bmd5\b", r"\bMD5\b", r"Message.*Digest.*5"],
    ...
    async def _analyze_vulnerability_details(
        ...
        if algorithm_type in ["asymmetric", "signature"] and any(
            alg in algorithm_lower for alg in ["rsa", "ecc", "dh"]
        ):
    ...
    def _get_replacement_recommendations(
        ...
        if threat_level in [QuantumThreatLevel.CRITICAL, QuantumThreatLevel.HIGH]:
            ...
            if "encryption" in usage_context.lower() or "kem" in usage_context.lower():
                ...
                "signature" in usage_context.lower() or "auth" in usage_context.lower()
            ):
    ...
    async def _generate_assessment_recommendations(
        ...
        critical_algs = [
            a for a in vulnerable_algs if a.threat_level == QuantumThreatLevel.CRITICAL
        ]
        if critical_algs:
            recommendations.append(
