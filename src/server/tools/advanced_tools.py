...
from ..initialization import get_km_client, get_metadata_extractor, get_smart_filter

if TYPE_CHECKING:
    from ...integration.macro_metadata import EnhancedMacroMetadata
...
async def km_search_macros_advanced(
    ...
    IMPORTANT: For list parameters (action_categories, complexity_levels):
    - Use array syntax: ["application_control", "text_manipulation"]
    - NOT string syntax: "[\"application_control\", \"text_manipulation\"]"
    """
    if ctx:
    ...
    try:
        ...
        macros_result = await km_client.list_macros_async(
            group_filters=None,  # Get macros from all groups for search
            enabled_only=True,  # Only search enabled macros by default
        )
            ...
            result_macros.append(
                {
                    ...
                    "usage_stats": {
                        ...
                        "last_executed": macro.usage_stats.last_executed.isoformat()
                        if macro.usage_stats.last_executed
                        else None,
