...
async def km_ai_status(
    ...
    try:
        ...
        if include_statistics:
            ...
            recent_usage = (
                ai_manager.usage_history[-10:] if ai_manager.usage_history else []
            )
            ...
            if ai_manager.usage_history:
                ...
                avg_processing_time = sum(
                    record["processing_time"] for record in ai_manager.usage_history
                ) / len(ai_manager.usage_history)
                response["statistics"] = {
                    "total_cost": total_cost,
                    "average_processing_time": avg_processing_time,
                    "total_tokens": sum(
                        record["tokens_used"] for record in ai_manager.usage_history
                    ),
                }
...
async def km_ai_intelligence(
    ...
    try:
        ...
        from ..ai.intelligent_automation import (
        ...
        if isinstance(input_data, dict):
            ...
            return {
                ...
                "context_summary": {
                    ...
                    "dimensions_analyzed": [dim.value for dim in dimensions],
...
async def km_ai_batch(
    ...
    try:
        ...
        from ..ai.batch_processing import (
            BatchJob,
            BatchMode,
            BatchProcessor,
            BatchTask,
...
async def km_ai_cost_optimization(
    ...
    try:
        ...
        from ..ai.cost_optimization import (
            BudgetId,
            BudgetPeriod,
            CostBudget,
        ...
        if operation == "budget":
            ...
            if result.is_left():
                return {"success": False, "error": str(result.get_left())}
...
async def km_ai_models(
    ...
    try:
        ...
        return {
            ...
            "filters_applied": {"provider": provider, "operation": operation},
            "timestamp": datetime.now(UTC).isoformat(),
        }
