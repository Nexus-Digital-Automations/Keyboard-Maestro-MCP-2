...
from ...core.errors import ErrorContext, ValidationError
    ...
    try:
        ...
        if report_format.lower() == "summary":
            ...
            formatted_data = {
                ...
                "top_recommendations": report_data["recommendations"][
                    :3
...
async def _handle_status_operation(audit_system, ctx: Context | None) -> dict[str, Any]:
    ...
    try:
        ...
        if event_stats.get("events_failed", 0) > 0:
            failure_rate = (
                event_stats.get("events_failed", 0)
                / max(event_stats.get("events_logged", 1), 1)
            ) * 100
            if failure_rate > 5:
