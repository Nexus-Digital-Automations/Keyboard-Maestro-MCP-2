...
async def km_dictionary_manager(
    ...
    json_data: Annotated[
        dict[str, Any] | None,
        Field(default=None, description="JSON data for bulk import operations"),
    ] = None,
    ctx: Context = None,
) -> dict[str, Any]:
    ...
    try:
        ...
        if (
            operation in ["get", "set", "delete", "list_keys", "export"]
            and not dictionary
        ):
            raise ValidationError(
                "dictionary",
                dictionary,
                f"Dictionary name required for {operation} operation",
            )

        if operation in ["get", "set", "delete"] and operation != "delete" and not key:
            raise ValidationError("key", key, f"Key required for {operation} operation")

        if operation == "set" and value is None:
            raise ValidationError("value", value, "Value required for set operation")
...
async def _create_dictionary(
    ...
    if not name or not all(c.isascii() and (c.isalnum() or c in "_- ") for c in name):
        raise ValidationError(
            "name",
            name,
            "Dictionary name must contain only alphanumeric characters, spaces, hyphens, and underscores",
        )
...
async def _list_dictionary_keys(
    ...
    keys = ["theme", "language", "auto_save", "last_backup", "window_position"]
