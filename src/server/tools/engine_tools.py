    ...
    try:
        ...
        if operation == "calculate" and not expression:
            raise ValidationError(
                "expression", expression, "Expression required for calculate operation"
            )
        ...
        if operation == "search_replace":
            ...
            if not text:
                raise ValidationError(
                    "text", text, "Text required for search_replace operation"
                )
        ...
        if operation == "reload":
            ...
            return await _search_replace(
                km_client, text, search_pattern, replace_pattern, use_regex, ctx
            )
        else:
            raise ValidationError(
...
async def _calculate_expression(
    km_client, expression: str, ctx: Context = None
) -> dict[str, Any]:
    ...
    if any(
        ...
        raise ValidationError(
            "expression", expression, "Expression contains forbidden operations"
        )
    ...
    try:
        ...
        if not all(c in allowed_chars for c in expression):
            raise ValidationError(
                "expression", expression, "Expression contains invalid characters"
            )
        ...
        raise ValidationError("expression", expression, f"Calculation failed: {str(e)}")


async def _process_tokens(
    km_client, token_string: str, ctx: Context = None
) -> dict[str, Any]:
    ...
    for match in re.finditer(date_pattern, token_string):
        ...
        processed = processed.replace(
            match.group(0), datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        )
...
async def _search_replace(
    ...
    try:
        ...
        raise ValidationError(
            "search_pattern", search_pattern, f"Invalid regex pattern: {str(e)}"
        )
