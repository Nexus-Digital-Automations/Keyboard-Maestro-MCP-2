...
async def km_interface_automation(
    ...
    key_combination: list[str] | None = None,
    button: str = "left",
    click_count: int = 1,
    drag_destination: dict[str, int] | None = None,
...
async def _handle_mouse_drag(
    coordinates: dict[str, int] | None,
    drag_destination: dict[str, int] | None,
    duration_ms: int,
    smooth_movement: bool,
    button: str,
    validate_coordinates: bool,
...
async def _handle_type_text(
    ...
    try:
        result = await keyboard_controller.type_text(text_content, delay_between_chars)
...
async def km_validate_coordinates(
    ...
    try:
        ...
        return {
            ...
            "error_message": safety_result.get_left().message
