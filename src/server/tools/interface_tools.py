...
async def km_interface_automation(
    operation: Annotated[
        ...
        Field(description="Interface automation operation"),
    ],
    ...
    duration_ms: Annotated[
        int,
        Field(default=200, ge=50, le=2000, description="Duration for drag operations"),
    ] = 200,
    modifiers: Annotated[
        list[Literal["cmd", "ctrl", "option", "shift"]] | None,
        Field(default=None, description="Modifier keys to hold during operation"),
    ] = None,
    ctx: Context = None,
) -> dict[str, Any]:
        ...
        if operation in ["click", "double_click", "right_click", "move_mouse"]:
            if not coordinates:
                raise ValidationError(
                    "coordinates",
                    None,
                    f"Coordinates required for {operation} operation",
                )
        ...
        if operation == "drag":
            if not coordinates or not end_coordinates:
                raise ValidationError(
                    "coordinates",
                    coordinates,
                    "Both start and end coordinates required for drag",
                )
            _validate_coordinates(coordinates)
            _validate_coordinates(end_coordinates)

        if operation == "type" and not text:
            raise ValidationError("text", text, "Text required for type operation")
...
async def _perform_click(
    ...
    modifier_str = ""
    if modifiers:
        modifier_map = {"cmd": "⌘", "ctrl": "⌃", "option": "⌥", "shift": "⇧"}
...
async def _type_text(_km_client: Any, text: str, ctx: Context = None) -> dict[str, Any]:
...
async def _press_keys(
    ...
    for key in keys:
        if key in valid_modifiers:
            ...
            raise ValidationError("key", key, f"Invalid key: {key}")

    if not main_key and len(modifiers) == 0:
        raise ValidationError("keystroke", keystroke, "No valid key specified")
