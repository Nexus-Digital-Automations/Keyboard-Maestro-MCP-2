...
_automation_hub: AutomationHub | None = None
...
def validate_sensor_identifiers(
    identifiers: list[str],
) -> Either[ValidationError, list[SensorId]]:
...
def validate_device_parameters(
                ...
                if reading_result.is_success():
                    ...
                    if alert_thresholds and real_time_alerts:
                        for threshold_name, threshold_value in alert_thresholds.items():
                            if (
                                ...
                                and reading.value > threshold_value
                            ):
                                alert = {
                                    "sensor_id": sensor_id,
                                    ...
                                    "severity": "warning"
                                    if reading.value < threshold_value * 1.2
                                    else "critical",
                                }
                    ...
                    if trigger_conditions:
                        for condition_config in trigger_conditions:
                            ...
                            if (
                                condition_config.get("sensor_id") == sensor_id
                                and "threshold" in condition_config
                                ...
                                if (
                                    (operator == ">" and reading.value > threshold)
                                    or (operator == "<" and reading.value < threshold)
                                    or (
                                        operator == "="
                                        and abs(reading.value - threshold) < 0.01
                                    )
                                ):
                                    action = {
...
async def km_manage_smart_home(
    ...
    try:
        ...
        if operation == "activate_scene":
            ...
            # Activate scene
            result = await automation_hub.activate_scene(
                scene_id,
                {
            ...
            if result.is_success():
                ...
                return {
                    ...
                    "adaptive_automation": adaptive_automation,
                    "execution_time_ms": (
                        datetime.now(UTC) - operation_start
                    ).total_seconds()
                    * 1000,
                    "timestamp": datetime.now(UTC).isoformat(),
                }
    ...
    try:
        ...
        if not device_sequence or len(device_sequence) > 50:
            return {
                ...
                "device_sequence_length": len(device_sequence)
                if device_sequence
                else 0,
            }
        ...
        if execution_result.is_success():
            ...
            coordination_status = {
                ...
                "error_recovery_used": fault_tolerance
                and execution_data.get("errors", 0) > 0,
            }
