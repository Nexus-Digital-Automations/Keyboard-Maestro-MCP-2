...
def _update_performance_metrics(operation: str, response_time: float):
    ...
    total_ops = (
        tool_performance_metrics["total_commands"]
...
async def km_process_natural_command(
    ...
    try:
        ...
        if text_result.is_left():
            return {
                "success": False,
                "error": text_result.get_left().message,
                "error_code": text_result.get_left().error_code,
            }

        text_content = text_result.get_right()
        ...
        if lang_result.is_left():
            return {
                "success": False,
                "error": lang_result.get_left().message,
                "error_code": lang_result.get_left().error_code,
            }

        language_code = lang_result.get_right()
        ...
        if result.is_left():
            return {
                "success": False,
                "error": result.get_left().message,
                "error_code": result.get_left().error_code,
            }

        processed_command = result.get_right()
...
async def km_recognize_intent(
    ...
    try:
        ...
        recognized_intents = result.get_right()
...
async def km_generate_from_description(
    ...
    try:
        ...
        if workflow_result.is_left():
            return {
                "success": False,
                "error": workflow_result.get_left().message,
                "error_code": workflow_result.get_left().error_code,
            }

        generated_workflow = workflow_result.get_right()
        ...
        if generate_documentation:
            response["documentation"] = {
                ...
                "steps": [
                    ...
                    {
                        "step": 2,
                        "action": "Execute main logic",
                        "description": "Perform primary workflow actions",
                    },
                    {
                        "step": 3,
                        "action": "Handle completion",
                        "description": "Process results and cleanup",
                    },
                ],
                "usage_notes": [
                    "Test workflow in safe environment",
...
async def km_conversational_interface(
    ...
    try:
        ...
        if conversation_result.is_left():
            return {
                "success": False,
                "error": conversation_result.get_left().message,
                "error_code": conversation_result.get_left().error_code,
            }

        conversation_response = conversation_result.get_right()
