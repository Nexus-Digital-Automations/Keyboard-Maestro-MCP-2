...
class PerformanceMonitorTools:
    ...
    def register_tools(self, mcp: FastMCP) -> None:
        ...
        async def km_monitor_performance(
            ...
            if metrics_types is None:
                metrics_types = ["cpu", "memory", "execution_time"]
            try:
        ...
        async def km_analyze_bottlenecks(
            ...
            """
            if bottleneck_types is None:
                bottleneck_types = ["cpu", "memory", "io", "network"]
            try:
        ...
        async def km_optimize_resources(
            ...
            monitoring_period: Annotated[
                int,
                Field(
                    description="Post-optimization monitoring period (seconds)",
                    ge=60,
                    le=3600,
                ),
            ] = 300,
            ...
            try:
                ...
                invalid_resources = [
                    r for r in target_resources if r not in valid_resources
                ]
                ...
                if optimization_results:
                    ...
                    session_result = (
                        await self.metrics_collector.start_monitoring_session(config)
                    )
                    ...
                    "recommendations": [
                        {
                            ...
                            if auto_apply
                            and any(
                                r["recommendation_id"] == rec.get("id")
                ...
                for session_id in active_sessions:
                    ...
                    if metrics_result.is_right():
                        ...
                        if metrics.metrics:
                            ...
                            dashboard_data["performance_metrics"][session_id] = (
                                latest_metrics
                            )
    ...
    def _format_alert_summary(self, metrics) -> dict[str, Any]:
        ...
        return {
            ...
            "most_recent": metrics.alerts[-1].triggered_at.isoformat()
            if metrics.alerts
            else None,
        }
    ...
    def _calculate_estimated_improvements(
        ...
        cpu_improvement = sum(
            ...
            for r in recommendations
            if "cpu" in r.get("type", "")
        )
        memory_improvement = sum(
            r.get("expected_improvement", 0)
            for r in recommendations
            if "memory" in r.get("type", "")
        )
    ...
    async def _test_alert_threshold(
        ...
        return {
            ...
            "recommendation": "Threshold appears appropriate"
            if not would_trigger
            else "Consider adjusting threshold to reduce false positives",
        }
...
_performance_monitor_tools: PerformanceMonitorTools | None = None
