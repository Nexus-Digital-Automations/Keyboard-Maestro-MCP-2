...
usage_forecaster: UsageForecaster | None = None
...
def _update_performance_metrics(operation: str, response_time: float) -> None:
    ...
    total_ops = (
        tool_performance_metrics["total_predictions"]
...
async def km_predict_automation_patterns(
    ...
    try:
        ...
        # Map string pattern types to enum values (defensive against import issues)
        try:
            # Try to use enum values if available, fallback to strings for testing
            if hasattr(PatternType, "USAGE_PATTERNS"):
                ...
                # Fallback for testing scenarios where enums aren't available
                pattern_type_mapping = {
                    "usage": "USAGE_PATTERNS",
                    "performance": "PERFORMANCE_PATTERNS",
                    "errors": "ERROR_PATTERNS",
                    "workflow": "WORKFLOW_PATTERNS",
                    "resource": "RESOURCE_CONSUMPTION",
                    "seasonal": "SEASONAL_BEHAVIOR",
                }
            pattern_enums = [
                pattern_type_mapping[pt]
                for pt in pattern_types
                if pt in pattern_type_mapping
            ]
        except Exception as e:
            return {
                "success": False,
        ...
        if pattern_result.is_left():
            error_info = pattern_result.get_left()
            error_message = getattr(error_info, "message", str(error_info))
            return {
                "success": False,
                "error": f"Pattern detection failed: {error_message}",
                "timestamp": start_time.isoformat(),
            }
                ...
                predictions.append(
                    {
                        ...
                        "prediction_values": prediction.predicted_values[
                            :10
                        ],  # Limit for response size
                        "confidence_intervals": prediction.confidence_intervals[:10]
        ...
        result = {
            ...
            "performance": {
                ...
                "patterns_per_second": len(detected_patterns)
                / max(response_time, 0.001),
            },
...
async def km_forecast_resource_usage(
    ...
    try:
        ...
        for resource_str in resource_types:
            ...
            if forecast_result.is_right():
                ...
                forecasts.append(
                    {
                        ...
                        "forecast_timestamps": [
                            ts.isoformat() for ts in forecast.forecast_timestamps[:20]
                        ],
                ...
                if capacity_planning:
                    max_usage = (
                        max(forecast.predicted_usage)
                        if forecast.predicted_usage
                        else forecast.current_usage
                    )
                    current_capacity = (
                        forecast.current_usage / 0.7
                        if forecast.current_usage > 0
                        else 100.0
                    )
        ...
        # Generate summary
        await usage_forecaster.get_forecasting_summary()
...
async def km_generate_insights(
    ...
    if insight_types is None:
        insight_types = ["optimization", "efficiency", "cost"]
    start_time = datetime.now(UTC)
    ...
    try:
        ...
        insight_categories = [
            insight_category_mapping[it]
            for it in insight_types
            if it in insight_category_mapping
        ]
        ...
        for insight in insights[:10]:  # Limit to top 10 insights
            insight_data = {
                ...
                if include_actionable_recommendations
                else [],
        ...
        await insight_generator.get_insight_summary()
    ...
    try:
        ...
        try:
            ...
            if hasattr(PatternType, "PERFORMANCE_PATTERNS") and hasattr(
                ...
                # Fallback for testing scenarios where enums aren't available
                pattern_types = ["PERFORMANCE_PATTERNS", "USAGE_PATTERNS"]

            pattern_result = await pattern_predictor.detect_patterns(
                features=sample_features,
                pattern_types=pattern_types,
                min_confidence=0.5
                if trend_detection_sensitivity == "low"
                else 0.7
                if trend_detection_sensitivity == "medium"
                else 0.8,
            )
        except Exception as e:
            return {
            ...
            if predict_trend_continuation:
                ...
                if pred_result.is_right():
                    ...
                    trend_analysis["trend_prediction"] = {
                        "predicted_direction": "continuing"
        ...
        statistical_summary = {
            ...
            "average_confidence": sum([t.get("trend_confidence", 0) for t in trends])
            / len(trends)
            if trends
            else 0,
        }
...
async def _generate_sample_trend_data(scope: str, period: str) -> dict[str, Any]:
    ...
    timestamps = [
        datetime.now(UTC) - timedelta(days=i) for i in range(data_points, 0, -1)
    ]
...
def _generate_pattern_recommendations(patterns: list) -> list[str]:
    ...
    high_confidence_patterns = [
        p
        for p in patterns
        if hasattr(p, "confidence_score") and p.confidence_score > 0.8
    ]
...
def _generate_capacity_recommendations(
    ...
    at_capacity = [
        ca
        for ca in capacity_analyses
        if ca.get("capacity_status") in ["at_capacity", "over_capacity"]
    ]
    if at_capacity:
        recommendations.append(
    ...
    approaching_capacity = [
        ca
        for ca in capacity_analyses
        if ca.get("capacity_status") == "approaching_limit"
    ]
...
def _generate_trend_insights(
    ...
    positive_trends = [
        t
        for t in trends
        if t.get("trend_direction") == "increasing" and t.get("trend_strength", 0) > 0.7
    ]
    if positive_trends:
        insights.append(
    ...
    negative_trends = [
        t
        for t in trends
        if t.get("trend_direction") == "decreasing" and t.get("trend_strength", 0) > 0.7
    ]
    if negative_trends:
        insights.append(
