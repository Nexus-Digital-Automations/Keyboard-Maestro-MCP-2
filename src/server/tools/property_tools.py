...
async def km_manage_macro_properties(
    operation: Annotated[
        Literal["get", "update"],
        Field(description="Operation to perform: get or update properties"),
    ],
    macro_id: Annotated[
        str,
        Field(description="Macro UUID or name to manage", min_length=1, max_length=255),
    ],
    properties: Annotated[
        dict[str, Any] | None,
        Field(default=None, description="Properties to update (for update operation)"),
    ] = None,
    ctx: Context = None,
) -> dict[str, Any]:
...
async def _get_macro_properties(
    ...
    for macro in macros:
        if (
            macro.get("id", "") == macro_id
            or macro.get("name", "").lower() == macro_id_lower
        ):
            target_macro = macro
            break
...
async def _update_macro_properties(
    ...
    if "color" in properties:
        ...
        if color and not (
            color.startswith("#")
            or color in ["red", "blue", "green", "yellow", "orange", "purple"]
        ):
