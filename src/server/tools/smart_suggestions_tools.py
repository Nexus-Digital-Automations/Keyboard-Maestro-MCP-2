...
class SmartSuggestionsManager:
    ...
    ) -> Either[SuggestionError, list[IntelligentSuggestion]]:
    ...
    try:
        ...
        if operation == "optimize":
            return await _handle_optimize_operation(
                user_id,
                context,
                priority_mapping[priority_level],
                max_suggestions,
                privacy_enum,
            )
...
async def _handle_configure_operation(
    ...
    try:
        ...
        config = {
            ...
            "ai_processor_available": _smart_suggestions_manager.ai_processor
            is not None,
            "privacy_level": privacy_level.value,
