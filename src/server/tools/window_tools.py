...
async def _handle_arrange_operation(
    ...
    try:
        ...
        if result.is_right():
            ...
            window_info_source = "pre_operation"
...
def _is_valid_app_identifier(identifier: str) -> bool:
    ...
    return all(not re.search(pattern, identifier) for pattern in suspicious_patterns)
...
def _format_window_info(window_info: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "position": {"x": window_info.position.x, "y": window_info.position.y},
        "size": {"width": window_info.size.width, "height": window_info.size.height},
...
def _format_screen_info(screen_info: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "origin": {"x": screen_info.origin.x, "y": screen_info.origin.y},
        "size": {"width": screen_info.size.width, "height": screen_info.size.height},
        "bounds": {
