...
from ...workflow.component_library import ComponentCategory, get_component_library
    ...
    def register_tools(self, mcp: FastMCP) -> None:
        ...
        async def km_create_visual_workflow(
            ...
            try:
                ...
                if workflow_result.is_left():
                    return (
                        f"Error: Failed to create workflow - {workflow_result.left()}"
                    )
        ...
        async def km_add_workflow_component(
            ...
            try:
                ...
                try:
                    properties_dict = (
                        json.loads(properties) if properties != "{}" else {}
                    )
                ...
                response = {
                    ...
                    "workflow_info": {"total_components": "updated_count_needed"},
                    "next_steps": [
            ...
            try:
                ...
                if title or description or properties_dict:
                    ...
                    updated_properties = ComponentProperties(
                        ...
                        else current_component.properties.title,
                        description=description
                        if description is not None
                        else current_component.properties.description,
            ...
            try:
                ...
                response = {
                    ...
                    "export_result": {
                        ...
                        "enabled_on_creation": enable_on_creation
                        if export_format == "macro"
                        else False,
                    },
                    "workflow_summary": {
                        "name": workflow.name,
                        ...
                        "complexity_score": min(
                            len(workflow.components) * 2 + len(workflow.connections),
                            100,
                        ),
                    },
                    "validation_report": {
                    ...
                    "next_steps": [
                        "Macro is ready for use in Keyboard Maestro"
                        if export_format == "macro"
                        else f"Export saved as {export_format}",
                ...
                for template_id, template in WORKFLOW_TEMPLATES.items():
                    template_info = {
                        ...
                        "preview_components": [
                            ...
                            for comp in template.component_definitions[
                                :5
        ...
        async def km_validate_workflow(
            ...
            try:
                ...
                if check_performance:
                    performance_metrics = {
                        "estimated_execution_time_ms": len(workflow.components)
                        * 100,  # Rough estimate
                        "memory_usage_estimate_kb": len(workflow.components) * 50,
                        "complexity_score": len(workflow.components) * 2
                        + len(workflow.connections),
                        "optimization_potential": "medium"
                        if len(workflow.components) > 10
                        else "low",
                    }
                ...
                response = {
                    ...
                    "workflow_statistics": {
                        ...
                        "component_types": {
                            comp_type.value: sum(
                                1
                                for c in workflow.components.values()
                    ...
                    "next_steps": [
                        "Fix any validation errors before exporting"
                        if validation_errors
                        else "Workflow validation passed",
...
_workflow_designer_tools: WorkflowDesignerTools | None = None
