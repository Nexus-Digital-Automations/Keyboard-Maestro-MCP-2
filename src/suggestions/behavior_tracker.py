...
class BehaviorTracker:
    ...
    async def track_automation_performance(
        ...
        try:
            ...
            execution_time = (
                execution_result.get("execution_time_ms", 0.0) / 1000.0
            )  # Convert to seconds
    ...
    def _find_matching_pattern(
        ...
        for pattern in patterns:
            ...
            if (
                action in pattern.action_sequence
                and len(pattern.context_tags & context_tags) > 0
            ):
                return pattern

            # Also check for exact action sequence match
            if (
                len(pattern.action_sequence) == 1
                and pattern.action_sequence[0] == action
            ):
                return pattern
    ...
    def _replace_pattern(
        self,
        user_id: str,
        old_pattern: UserBehaviorPattern,
        new_pattern: UserBehaviorPattern,
    ) -> None:
    ...
    def _update_performance_metrics(
        self,
        existing: AutomationPerformanceMetrics,
        success: bool,
        ...
        # Update success rate (moving average)
        new_success_rate = (
            existing.success_rate * existing.execution_count + (1.0 if success else 0.0)
        ) / new_count

        # Update execution time (moving average)
        new_execution_time = (
            existing.average_execution_time * existing.execution_count + execution_time
        ) / new_count

        # Update error frequency (moving average)
        new_error_frequency = (
            existing.error_frequency * existing.execution_count
        ...
        try:
            ...
            if context.get("success", True):
                ...
                prefs["success_patterns"] = [
                    p for p in prefs["success_patterns"] if p["timestamp"] > cutoff
                ]
    ...
    async def _cleanup_old_patterns(self, user_id: str) -> None:
        ...
        try:
            ...
            for pattern in patterns:
                ...
                if (
                    pattern.last_observed > cutoff_date
                    or pattern.frequency >= 5
                    or pattern.get_efficiency_score() > 0.8
                ):
                    filtered_patterns.append(pattern)
    ...
    def _calculate_activity_trend(self, user_id: str, days: int) -> str:
        ...
        try:
            ...
            recent_actions = [
                action
                for action in self.action_sequences[user_id]
