...
from src.core.suggestion_system import (
    ...
    PriorityLevel,
    SuggestionContext,
    SuggestionSecurityValidator,
    SuggestionType,
)

if TYPE_CHECKING:
    from src.suggestions.pattern_analyzer import (
...
class AIPromptGenerator:
    ...
    def generate_tool_recommendation_prompt(
        ...
        context_summary = context.get_context_summary()
        active_tools_text = (
            ", ".join(list(context.active_tools)[:5])
        ...
        tools_sample = (
            available_tools[:15] if len(available_tools) > 15 else available_tools
        )
...
class RecommendationEngine:
    ...
    async def _generate_workflow_optimizations(
        ...
        try:
            ...
            optimization_insights = [
                ...
                in ["low_efficiency", "low_reliability", "context_problem"]
            ]
        ...
        try:
            if insight.insight_type == "low_efficiency":
                return IntelligentSuggestion(
                    ...
                    description="Several workflows are running slower than optimal. "
                    "Consider adding conditions or reducing complexity.",
    ...
    def _create_underutilized_tool_suggestion(
        ...
        try:
            ...
            underutilized_tools = [
                (
                    "km_control_flow",
                    "Advanced workflow logic with if/then/else conditions",
                ),
                (
                    "km_create_trigger_advanced",
                    "Event-driven automation that responds automatically",
                ),
    ...
    def _generate_cache_key(
        ...
        types_str = (
            ",".join(sorted([t.value for t in suggestion_types]))
            if suggestion_types
            else "all"
        )
