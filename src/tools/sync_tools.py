...
def register_sync_tools(mcp) -> None:
    ...
    async def km_start_realtime_sync(
        ...
        try:
            ...
            from ..server_utils import get_file_monitor, get_sync_manager
            ...
            return {
                ...
                "data": {
                    ...
                    "file_monitoring": {
                        ...
                        "details": get_file_monitor().get_status()
                        if file_monitor_status == "active"
                        else None,
                    },
                    "configuration": {
                        ...
                        "cache_ttl_minutes": sync_mgr.config.cache_ttl.total_seconds()
                        / 60,
                    },
                },
                "metadata": {
    ...
    async def km_sync_status(
        ...
        try:
            ...
            is_healthy = (
                sync_status["status"] in ["active", "stopped"]
                and sync_status["consecutive_errors"] < 3
            )
    ...
    async def km_force_sync(
        ...
        try:
            # Import here to avoid circular dependencies
            from ..server_utils import get_sync_manager
