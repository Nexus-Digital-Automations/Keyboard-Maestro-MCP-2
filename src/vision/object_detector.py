...
class ObjectDetector:
    ...
    async def _simulate_object_detection(
        ...
        for i in range(num_objects):
            ...
            obj = DetectedObject(
                ...
                features=[
                    "color_primary",
                    "shape_rectangular" if width / height > 1.5 else "shape_square",
    ...
    async def _post_process_detections(
        ...
        if self.config.class_filter:
            class_filtered = [
                obj for obj in limited if obj.class_name in self.config.class_filter
            ]
    ...
    async def _update_object_tracking(self, detections: list[DetectedObject]) -> None:
        ...
        for detection in detections:
            ...
            for track_id, track in self.object_tracks.items():
                if (
                    track.object_category == detection.category
                    and track.is_active
                    and track.position_history
                ):
                    ...
                    last_center = (
                        last_pos.x + last_pos.width / 2,
                        last_pos.y + last_pos.height / 2,
                    )
                    current_center = (
                        current_pos.x + current_pos.width / 2,
                        current_pos.y + current_pos.height / 2,
                    )
