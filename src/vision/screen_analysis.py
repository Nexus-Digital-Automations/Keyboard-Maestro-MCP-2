...
class WindowInfo:
    ...
    def is_visible(self) -> bool:
        ...
        return (
            self.state not in [WindowState.HIDDEN, WindowState.MINIMIZED]
...
class ScreenAnalysisEngine:
    ...
    def __init__(self, enable_privacy_protection: bool = True):
        ...
        self.privacy_protection = (
            PrivacyProtection() if enable_privacy_protection else None
        )
        self.capture_cache: dict[str, ScreenCapture] = {}
        self.window_cache: dict[str, tuple[list[WindowInfo], datetime]] = {}
        self.change_detection_baseline: ScreenCapture | None = None
        self.analysis_stats = {
            "total_captures": 0,
            "cache_hits": 0,
    ...
    async def capture_screen_region(
        ...
        try:
            ...
            # Check permissions
            permission_check = (
                await self.permission_manager.check_screen_recording_permission()
            )
            ...
            if privacy_mode and self.privacy_protection:
                sensitive_regions = [
                    w.bounds
                    for w in windows
                    if self.privacy_protection.should_filter_window(w)
                    and region.overlaps_with(w.bounds)
                ]
            ...
            self.analysis_stats["average_capture_time"] = (
...
async def find_active_window() -> Either[VisualError, WindowInfo | None]:
