...
class AuthenticationCredentials:
    ...
    def __post_init__(self):
        ...
        elif self.auth_type == AuthenticationType.CUSTOM_HEADER and (
            "header_name" not in self.credentials
    ...
    def _sanitize_credentials(
        ...
        try:
            if auth_type == AuthenticationType.API_KEY:
                ...
                clean_value = "".join(
                    char for char in header_value if ord(char) >= 32 or char in "\t\n\r"
                )
...
def create_bearer_token_auth(
    token: str,
) -> Either[ValidationError, AuthenticationCredentials]:
    """Create Bearer token authentication."""
    return AuthenticationManager.create_authentication("bearer_token", {"token": token})
