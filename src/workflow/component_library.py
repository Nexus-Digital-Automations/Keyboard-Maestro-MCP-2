...
class ComponentLibrary:
        ...
        self.component_definitions.update(
            {
                ...
                "condition_file_exists": create_component_definition(
                    ...
                    {"file_path": "", "check_type": "exists", "follow_aliases": True},
                    ComponentCategory.CONDITIONS,
                    icon="document",
                    color="#FF6B35",
                ),
                "condition_app_running": create_component_definition(
                    ...
                    {"bundle_id": "", "app_name": "", "check_type": "is_running"},
                    ComponentCategory.CONDITIONS,
                    icon="app",
                    color="#FF6B35",
                ),
        ...
        self.component_definitions.update(
            {
                ...
                "action_delete_file": create_component_definition(
                    ...
                    {"file_path": "", "confirm_deletion": True, "secure_delete": False},
                    ComponentCategory.ACTIONS,
                    ActionCategory.FILE_OPERATIONS.value,
                    icon="trash",
                    color="#FF3B30",
        ...
        self.component_definitions.update(
            {
                ...
                "action_menu_select": create_component_definition(
                    ...
                    {"app_name": "", "menu_path": [], "wait_for_completion": True},
                    ComponentCategory.ACTIONS,
                    ActionCategory.APPLICATION_CONTROL.value,
    ...
    def list_components_by_category(
        ...
        for key, definition in self.component_definitions.items():
            if definition["category"] == category:
                components.append({"key": key, **definition})
    ...
    def search_components(self, query: str) -> list[dict[str, Any]]:
        ...
        for key, definition in all_components.items():
            if (
                query_lower in definition["title"].lower()
                or query_lower in definition["description"].lower()
                or query_lower in definition["category"].value.lower()
                or (
                    definition.get("subcategory")
                    and query_lower in definition["subcategory"].lower()
                )
            ):
                results.append(
                    {
