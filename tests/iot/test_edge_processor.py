    ...
    async def test_processing_result_structure(self) -> None:
        ...
        async for result in self.processor.process_task_queue():
            if result.is_success():
                ...
                assert (
                    "patterns_detected"
                    in processing_result.result_data["analytics_results"]
                )
                assert (
                    "local_processing_efficiency"
                    in processing_result.result_data["edge_insights"]
                )
