...
class TestEngineProperties:
    ...
    def test_execution_always_returns_result(
        ...
        if result.status == ExecutionStatus.COMPLETED:
            assert result.completed_at is not None, (
                "Completion time must be set for completed executions"
            )
    ...
    def test_invalid_macros_rejected(self, context: ExecutionContext):
        ...
        assert result.status == ExecutionStatus.FAILED, (
            "Invalid macro should result in failed execution"
        )
        assert result.error_details is not None, (
            "Failed execution should have error details"
        )
        assert "invalid" in result.error_details.lower(), (
            "Error should indicate invalid macro"
        )
    ...
    def test_execution_respects_timeouts(self, macro_def: MacroDefinition):
        ...
        try:
            engine.execute_macro(macro_def, context)
            ...
            assert execution_time <= max_allowed_time, (
                f"Execution took {execution_time}s, max allowed {max_allowed_time}s"
            )
            ...
            assert execution_time <= 2.0, (
                f"Even failed execution took too long: {execution_time}s"
            )
    ...
    def test_permission_enforcement(self, available_permissions: frozenset[Permission]):
        ...
        if required_permission not in available_permissions:
            ...
            assert result.status == ExecutionStatus.FAILED, (
                "Should fail when missing required permissions"
            )
            # Check if any command results indicate permission denied
            permission_denied = any(
                not cmd_result.success
                and "permission" in (cmd_result.error_message or "").lower()
                for cmd_result in result.command_results
            )
            assert (
                permission_denied
                or "permission" in (result.error_details or "").lower()
    ...
    def test_execution_timing_consistency(self, expected_duration: Duration):
        ...
        assert max_deviation <= allowed_deviation, (
            f"Timing too inconsistent: {execution_times}"
        )
    ...
    def test_execution_status_transitions_valid(self, macro_def: MacroDefinition):
        ...
        assert result.status in valid_final_states, (
            f"Invalid final status: {result.status}"
        )
        ...
        if result.status == ExecutionStatus.COMPLETED:
            ...
            assert result.completed_at is not None, (
                "COMPLETED executions must have completion time"
            )

        if result.status == ExecutionStatus.FAILED:
            assert not result.is_successful(), (
                "FAILED executions must not be successful"
            )
    ...
    def test_engine_resource_cleanup(self, num_executions: int):
        ...
        assert final_active <= initial_active + 1, (
            "Active executions not properly cleaned up"
        )
