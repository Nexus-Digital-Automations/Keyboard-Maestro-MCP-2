...
from src.core.macro_editor import (
    ...
    EditOperation,
    MacroEditor,
    MacroEditorValidator,
    calculate_macro_complexity,
    calculate_macro_health,
)
...
class TestMacroEditorProperties:
    ...
    def test_add_action_properties(
        self,
        action_type: str,
        config: dict[str, Any],
        position: int,
    ...
    def test_debug_session_timeout_validation(self, timeout: float) -> None:
        """Property: Debug session timeout validation should work correctly."""
        if 1 <= timeout <= 300:
            # Should succeed
            debug_session = DebugSession(macro_id="test_macro", timeout_seconds=timeout)
            assert debug_session.timeout_seconds == timeout
        else:
            ...
            with pytest.raises(ContractViolationError):
                DebugSession(macro_id="test_macro", timeout_seconds=timeout)
    ...
    def test_debug_config_validation_properties(
        self,
    ...
    def test_macro_complexity_calculation_properties(self, macro_data: Any) -> None:
        ...
        if not macro_data or all(
            not macro_data.get(key) for key in ["actions", "triggers", "conditions"]
        ):
...
class TestMacroDebuggerProperties:
    ...
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
    @pytest.mark.asyncio
    async def test_step_execution_properties(
        self,
        debugger: Awaitable[Any] | Any,
        step_count: int,
    ) -> None:
...
class TestMacroEditorToolProperties:
    ...
    @pytest.mark.asyncio
    async def test_empty_macro_identifier_validation(
        self,
        empty_identifier: str,
    ) -> None:
