...
from src.plugins.api_bridge import PluginAPIBridge, RateLimiter
...
from src.plugins.security_sandbox import SecurityLimits
...
class TestAPIBridgeProperties:
    ...
    async def test_api_bridge_security_boundaries(
        ...
        if tool_name == "nonexistent_tool":
            ...
            error = result.get_left()
            assert (
                "permission" in error.message.lower()
                or "denied" in error.message.lower()
            )
    ...
    async def test_rate_limiting_enforcement(
        ...
        from src.core.plugin_architecture import PermissionId, SecurityProfile
        ...
        if call_count > 3:
            assert rate_limited, (
                f"Expected rate limiting with {call_count} calls, but made {successful_calls} successful calls"
            )
