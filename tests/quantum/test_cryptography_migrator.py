...
class TestQuantumReadinessAnalysis:
    ...
    async def test_analyze_quantum_readiness_application_scope(self) -> None:
        ...
        assert (
            "app_api_key" in str(assessment.quantum_vulnerable_assets)
            or len(assessment.quantum_vulnerable_assets) >= 0
        )
...
class TestMigrationPlanCreation:
    ...
    async def test_create_migration_plan_gradual(self) -> None:
        ...
        # Get actual asset IDs from discovered assets
        all_asset_ids = list(migrator.cryptographic_assets.keys())
        assert len(all_asset_ids) >= 2, (
            "Need at least 2 assets for gradual migration test"
        )
...
class TestMigrationPlanExecution:
    ...
    async def test_execute_migration_plan_metrics_update(self) -> None:
        ...
        assert (final_successful > initial_successful) or (
...
class TestMigrationIntegration:
    ...
    async def test_full_migration_workflow(self) -> None:
        ...
        if vulnerable_count > 0:
            ...
            target_assets = [
                asset.asset_id for asset in assessment.quantum_vulnerable_assets[:2]
            ]
        else:
    ...
    async def test_multiple_scope_analysis_consistency(self) -> None:
        ...
        total_assets = sum(
            len(assessment.quantum_vulnerable_assets)
...
class TestMigrationPerformance:
    ...
    async def test_migration_plan_creation_performance(self) -> None:
        ...
        await migrator.create_migration_plan(target_assets=target_assets)
