...
class TestKMExecuteMacro:
    ...
    async def test_execute_macro_timeout(self):
        ...
        with patch("src.server.tools.core_tools.get_km_client") as mock_get_client:
            ...
            result = await km_execute_macro(identifier="Slow Macro", timeout=1)
    ...
    async def test_execute_macro_invalid_identifier(self):
        ...
        assert result["success"] is False
        assert (
            "invalid" in result["error"].lower() or "empty" in result["error"].lower()
        )
    ...
    async def test_execute_macro_client_error(self):
        ...
        with patch("src.server.tools.core_tools.get_km_client") as mock_get_client:
            ...
            result = await km_execute_macro(identifier="Test Macro", timeout=30)
...
class TestKMVariableManager:
    ...
    async def test_set_variable_success(self):
        ...
        with patch("src.server.tools.core_tools.get_km_client") as mock_get_client:
            mock_client = Mock()
            mock_client.set_variable = AsyncMock(
                return_value={
                    "success": True,
    ...
    async def test_list_variables_success(self):
        ...
        with patch("src.server.tools.core_tools.get_km_client") as mock_get_client:
            mock_client = Mock()
            mock_client.list_variables = AsyncMock(
                return_value={
                    "success": True,
            ...
            result = await km_variable_manager(operation="list", scope="all")
    ...
    async def test_delete_variable_success(self):
        ...
        with patch("src.server.tools.core_tools.get_km_client") as mock_get_client:
            mock_client = Mock()
            mock_client.delete_variable = AsyncMock(
                return_value={
                    "success": True,
                    "variable_name": "testVar",
                    "deleted": True,
    ...
    async def test_invalid_operation(self):
        ...
        assert (
            ...
            or "operation" in result.get("error", "").lower()
        )
