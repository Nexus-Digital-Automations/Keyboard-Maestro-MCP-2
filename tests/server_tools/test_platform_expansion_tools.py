...
class TestPlatformExpansionFoundation:
    ...
    def sample_communication_data(self):
        ...
        return {
            ...
            "delivery_options": {"retry_count": 3, "timeout_seconds": 30},
        }
...
class TestPlatformExpansionIntegration:
    ...
    async def test_platform_tool_response_consistency(self, execution_context):
        ...
        platform_tools = [
            (
                "src.server.tools.communication_tools",
                "km_send_notification",
                {
                    "message_type": "info",
            ...
            (
                "src.server.tools.visual_tools",
                "km_capture_screen",
                {"region": {"x": 0, "y": 0, "width": 100, "height": 100}},
            ),
            (
                "src.server.tools.plugin_tools",
                "km_manage_plugins",
                {"operation": "list"},
            ),
        ]

        for module_name, tool_name, _test_params in platform_tools:
            try:
                module = __import__(module_name, fromlist=[tool_name])
                tool_func = getattr(module, tool_name)
