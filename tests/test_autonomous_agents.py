...
from src.agents.agent_manager import AgentManager, AutonomousAgent
from src.agents.communication_hub import (
    CommunicationHub,
    Message,
    MessagePriority,
    MessageType,
...
from src.agents.safety_validator import SafetyLevel, SafetyValidator
...
class TestAgentManager:
    ...
    async def test_multi_agent_coordination(self) -> None:
        ...
        # Register agents with communication hub for coordination
        await manager.communication_hub.register_agent(optimizer_id)
        await manager.communication_hub.register_agent(monitor_id)

        # Verify communication hub has both agents (broadcast + 2 direct channels = 3)
        assert len(manager.communication_hub.channels) >= 3
...
class TestLearningSystem:
    ...
    async def test_pattern_recognition(self) -> None:
        ...
        # Pattern discovery happens when similar experiences accumulate
        ...
        if len(learning_system.patterns) == 0:
            ...
            # If patterns were discovered automatically, verify characteristics
...
class TestResourceOptimizer:
    ...
    async def test_resource_allocation(self) -> None:
        ...
        # Request resources
        requirements = {ResourceType.CPU: 30.0, ResourceType.MEMORY: 40.0}
    ...
    async def test_resource_optimization(self) -> None:
        ...
        # High priority agent
        await optimizer.request_resources(agent1, {ResourceType.CPU: 60.0}, priority=9)

        # Low priority agent
        await optimizer.request_resources(agent2, {ResourceType.CPU: 30.0}, priority=3)
    ...
    async def test_resource_prediction(self) -> None:
        ...
        predictions = await optimizer.predict_requirements(agent_id, timedelta(hours=1))
...
def test_resource_allocation_properties(requirements: list[Any] | str) -> None:
    ...
    total_resources = dict.fromkeys(ResourceType, 100.0)
    optimizer = ResourceOptimizer(total_resources)
