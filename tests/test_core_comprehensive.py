...
class TestErrorHierarchy:
    ...
    def test_timeout_error_creation(self) -> bool:
        ...
        assert (
            "Operation 'macro_execution' timed out after 30.0 seconds" in error.message
        )
    ...
    def test_error_chain_creation(self) -> None:
        ...
        assert (
            "Execution failed for operation 'macro_execution'" in chained_error.message
        )
...
class TestEitherMonad:
    ...
    def test_either_fold_operations(self) -> None:
        ...
        right_result = (
            ...
            else f"Error: {right_value.get_left()}"
        )
        assert right_result == "Success: 42"

        # Test left value processing
        left_result = (
            f"Error: {left_value.get_left()}"
            if left_value.is_left()
            else f"Success: {left_value.get_right()}"
        )
...
class TestContractSystem:
    ...
    def test_require_decorator_failure(self) -> dict[str, Any]:
        ...
        assert any(
            msg in exc_info.value.condition
    ...
    def test_contract_violation_error_details(self) -> None:
        ...
        try:
            test_function(5)
            raise AssertionError("Should have raised ContractViolationError")
