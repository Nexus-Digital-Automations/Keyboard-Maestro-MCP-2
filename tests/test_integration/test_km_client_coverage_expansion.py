...
class TestKMClient:
    ...
    @pytest.fixture
    def km_client(self, mock_connection):
    ...
    @pytest.mark.asyncio
    async def test_execute_macro_by_id(self, km_client, mock_connection):
        ...
        with patch.object(km_client, "execute_macro") as mock_execute:
            mock_execute.return_value = Either.right(
                {"execution_id": "exec-123", "status": "completed", "duration": 1.5}
            )
    ...
    @pytest.mark.asyncio
    async def test_execute_macro_with_parameters(self, km_client):
    ...
    @pytest.mark.asyncio
    async def test_list_macros_with_filtering(self, km_client):
    ...
    @pytest.mark.asyncio
    async def test_trigger_management(self, km_client):
...
class TestScriptValidation:
    ...
    def test_script_content_validation(self):
        ...
        valid_script = 'tell application "Keyboard Maestro Engine" to execute macro "test"'
    ...
    def test_script_safety_checks(self):
        """Test script safety patterns."""
        # Dangerous patterns to avoid
        dangerous_patterns = ["rm -rf", "delete", "trash", "sudo"]
        
        safe_script = 'tell application "Keyboard Maestro Engine" to execute macro "safe"'
...
class TestURLSchemeValidation:
    ...
    def test_url_parameter_validation(self):
        ...
        assert all(isinstance(k, str) and isinstance(v, str) for k, v in valid_params.items())
    ...
    def test_url_scheme_patterns(self):
...
class TestErrorRecoveryPatterns:
    ...
    def test_fallback_service_patterns(self):
