...
class TestConnectionMethod:
    ...
    def test_connection_method_string_values(self):
...
class TestConnectionConfig:
    ...
    def test_config_creation_minimal(self):
        ...
        config = ConnectionConfig(
            method=ConnectionMethod.APPLESCRIPT, timeout=Duration(seconds=30)
        )
    ...
    def test_config_validation(self):
        ...
        short_timeout_config = ConnectionConfig(
            method=ConnectionMethod.APPLESCRIPT, timeout=Duration(seconds=1)
        )
...
class TestTriggerDefinition:
    ...
    def test_trigger_creation_hotkey(self):
    ...
    def test_trigger_creation_text_expansion(self):
...
class TestKMClient:
    ...
    def test_client_connection_check(self, mock_config):
    ...
    def test_macro_execution_simulation(self, mock_config):
        ...
        with patch.object(client, "execute_macro") as mock_execute:
            mock_execute.return_value = Either.right(
                {"execution_id": "exec-456", "status": "completed", "duration": 2.5}
            )
    ...
    def test_error_handling(self, mock_config):
        ...
        with patch.object(client, "check_connection") as mock_connect:
            mock_connect.return_value = Either.left(KMError(code="CONN_ERROR", message="Connection failed"))
...
class TestClientWithFallback:
    ...
    def test_create_client_with_fallback_primary_success(self):
        """Test fallback when primary method succeeds."""
        primary_config = ConnectionConfig(
            method=ConnectionMethod.APPLESCRIPT, timeout=Duration(seconds=30)
        )
        fallback_config = ConnectionConfig(
            method=ConnectionMethod.URL_SCHEME, timeout=Duration(seconds=30)
        )
    ...
    def test_create_client_with_fallback_fallback_needed(self):
        """Test fallback when primary method fails."""
        primary_config = ConnectionConfig(
            method=ConnectionMethod.WEB_API, timeout=Duration(seconds=10)
        )
        fallback_config = ConnectionConfig(
            method=ConnectionMethod.APPLESCRIPT, timeout=Duration(seconds=30)
        )
...
class TestRetryWithBackoff:
    ...
    def test_retry_success_on_first_attempt(self):
        ...
        result = retry_with_backoff(
            successful_operation, max_retries=3, initial_delay=Duration(seconds=0.1)
        )
    ...
    def test_retry_success_after_failures(self):
        ...
        def eventually_successful_operation():
            ...
            attempt_count += 1
            if attempt_count < 3:
                return Either.left(KMError(code="RETRY_ERROR", message=f"Attempt {attempt_count} failed"))
    ...
    def test_retry_exhausted_attempts(self):
        ...
        result = retry_with_backoff(
            always_failing_operation, max_retries=3, initial_delay=Duration(seconds=0.01)
        )
    ...
    def test_retry_with_backoff_parameters(self):
...
class TestKMError:
    ...
    def test_km_error_creation(self):
    ...
    def test_km_error_with_details(self):
    ...
    def test_km_error_attributes(self):
...
class TestAdvancedOperations:
    ...
    def test_macro_list_retrieval(self):
        """Test retrieving list of macros."""
        config = ConnectionConfig(
            method=ConnectionMethod.APPLESCRIPT, timeout=Duration(seconds=60)
        )
    ...
    def test_configuration_validation_comprehensive(self):
        ...
        valid_configs = [
            ...
            ConnectionConfig(
                method=ConnectionMethod.URL_SCHEME, timeout=Duration(seconds=15)
            ),
        ]

        for config in valid_configs:
            assert config.method in [ConnectionMethod.APPLESCRIPT, ConnectionMethod.WEB_API, ConnectionMethod.URL_SCHEME]
