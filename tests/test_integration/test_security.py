...
from src.integration.security import (
    ...
    get_minimum_security_level,
    is_sanitized,
    is_valid_km_format,
    process_km_event,
    sanitize_trigger_configuration,
...
class TestValidationResult:
    ...
    def test_unsafe_result_creation(self) -> None:
...
class TestInputValidation:
    ...
    def test_script_injection_detection(self, malicious_input: Any) -> None:
        ...
        assert not result.is_safe
        script_violations = [
            v for v in result.violations if v.threat_type == ThreatType.SCRIPT_INJECTION
        ]
    ...
    def test_path_traversal_detection(self, path_traversal_input: Any) -> None:
        ...
        path_violations = [
            v for v in result.violations if v.threat_type == ThreatType.PATH_TRAVERSAL
        ]
        assert len(path_violations) > 0
        # Check for path traversal pattern detection in violation descriptions
        assert any(
    ...
    def test_sql_injection_detection(self, malicious_input: Any) -> None:
        ...
        sql_violations = [
            v for v in result.violations if v.threat_type == ThreatType.SQL_INJECTION
        ]
    ...
    def test_applescript_danger_detection(self, malicious_input: Any) -> None:
        ...
        macro_violations = [
            v for v in result.violations if v.threat_type == ThreatType.MACRO_ABUSE
        ]
        assert len(macro_violations) > 0
        # Check for AppleScript pattern detection in violation descriptions
        assert any("applescript" in v.violation_text.lower() for v in macro_violations)
...
class TestTriggerDataSanitization:
    ...
    def test_sanitize_malicious_trigger_data(self, malicious_input: Any) -> None:
        ...
        assert "[BLOCKED_SCRIPT]" in config.get("script", "")  # Script blocked
        assert "[BLOCKED_SCRIPT]" in config.get("js", "")  # JavaScript blocked

    def test_permission_determination(self) -> None:
        """Test determination of required permissions."""
        hotkey_data = {"trigger_type": "hotkey", "configuration": {"key": "space"}}
    ...
    def test_clipboard_permission_detection(self) -> None:
    ...
    def test_network_permission_detection(self) -> None:
    ...
    def test_screen_capture_permission_detection(self) -> None:
        """Test detection of screen capture requirements."""
        screen_data = {
            "trigger_type": "system",
            "configuration": {"action": "screen capture", "area": "full screen"},
        }
...
class TestKMFormatValidation:
    ...
    def test_invalid_km_format_missing_fields(self) -> None:
    ...
    def test_invalid_km_format_wrong_types(self) -> None:
    ...
    def test_invalid_trigger_type(self) -> None:
        ...
        invalid_data = {"trigger_type": "invalid_type", "configuration": {}}
        assert not is_valid_km_format(invalid_data)
...
class TestSanitizationValidation:
    ...
    def test_is_sanitized_check(self) -> None:
...
class TestContractValidation:
    ...
    def test_process_km_event_invalid_format(self) -> None:
...
class TestTriggerManagementSecurity:
    ...
    def test_sanitize_trigger_configuration(self, malicious_input: dict[str, Any] | list[Any]) -> None:
        ...
        # Should be sanitized
        assert "[BLOCKED_SCRIPT]" in sanitized.get("script", "")


class TestSecurityUtilities:
    """Test security utility functions."""

    def test_create_security_report(self) -> None:
    ...
    def test_get_minimum_security_level(self) -> None:
...
@pytest.mark.parametrize(
    "trigger_type,expected_permission",
    [
