...
# Import interface types and errors
...
class TestInterfaceOperations:
    ...
    async def test_click_operation_success(
        self, mock_context, mock_km_client, valid_coordinates
    ):
        ...
        with patch(
            ...
            result = await km_interface_automation(
                operation="click", coordinates=valid_coordinates, ctx=mock_context
            )
    ...
    async def test_right_click_operation_success(
        ...
        with patch(
            ...
            result = await km_interface_automation(
                operation="right_click", coordinates=valid_coordinates, ctx=mock_context
            )
    ...
    async def test_move_mouse_operation_success(
        ...
        with patch(
            ...
            result = await km_interface_automation(
                operation="move_mouse", coordinates=valid_coordinates, ctx=mock_context
            )
    ...
    async def test_drag_operation_success(
        self, mock_context, mock_km_client, valid_coordinates, valid_end_coordinates
    ):
    ...
    @pytest.mark.asyncio
    async def test_type_operation_success(self, mock_context, mock_km_client):
        ...
        with patch(
            ...
            result = await km_interface_automation(
                operation="type", text=test_text, ctx=mock_context
            )
    ...
    @pytest.mark.asyncio
    async def test_key_press_operation_success(self, mock_context, mock_km_client):
        ...
        with patch(
            ...
            result = await km_interface_automation(
                operation="key_press", keystroke=keystroke, ctx=mock_context
            )
...
class TestInterfaceErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_missing_text_error(self, mock_context):
        """Test error when text is missing for type operation."""
        with patch("src.server.tools.interface_tools.get_km_client"):
            result = await km_interface_automation(
                operation="type", text=None, ctx=mock_context
            )
    ...
    @pytest.mark.asyncio
    async def test_missing_keystroke_error(self, mock_context):
        """Test error when keystroke is missing for key_press operation."""
        with patch("src.server.tools.interface_tools.get_km_client"):
            result = await km_interface_automation(
                operation="key_press", keystroke=None, ctx=mock_context
            )
    ...
    async def test_missing_drag_coordinates_error(
        self, mock_context, valid_coordinates
    ):
...
class TestCoordinateValidation:
    ...
    def test_missing_x_coordinate(self):
...
class TestHelperFunctions:
    ...
    @pytest.mark.asyncio
    async def test_perform_click_function(self, mock_context):
        ...
        result = await _perform_click(
            mock_km_client, "click", coordinates, modifiers, mock_context
        )
    ...
    @pytest.mark.asyncio
    async def test_move_mouse_function(self, mock_context):
    ...
    @pytest.mark.asyncio
    async def test_perform_drag_function(self, mock_context):
        ...
        result = await _perform_drag(
            mock_km_client, start_coords, end_coords, duration, modifiers, mock_context
        )
    ...
    @pytest.mark.asyncio
    async def test_type_text_function(self, mock_context):
    ...
    @pytest.mark.asyncio
    async def test_press_keys_function_simple(self, mock_context):
    ...
    @pytest.mark.asyncio
    async def test_press_keys_function_complex(self, mock_context):
    ...
    @pytest.mark.asyncio
    async def test_press_keys_invalid_key(self, mock_context):
...
class TestInterfaceIntegration:
    ...
    @pytest.mark.asyncio
    async def test_complete_interface_workflow(self, mock_context, mock_km_client):
        ...
        with patch(
            ...
            click_result = await km_interface_automation(
                operation="click", coordinates=coordinates, ctx=mock_context
            )
            assert click_result["success"] is True

            # Test type
            type_result = await km_interface_automation(
                operation="type", text="Hello World", ctx=mock_context
            )
            assert type_result["success"] is True

            # Test key press
            key_result = await km_interface_automation(
                operation="key_press", keystroke="cmd+a", ctx=mock_context
            )
            assert key_result["success"] is True

    @pytest.mark.asyncio
    async def test_complex_drag_operation(self, mock_context, mock_km_client):
...
class TestInterfaceContext:
    ...
    async def test_context_error_logging(self, mock_context, mock_km_client):
        ...
        with patch(
            ...
            result = await km_interface_automation(
                operation="click", coordinates={"x": 100, "y": 200}, ctx=mock_context
            )

            assert result["success"] is False

    @pytest.mark.asyncio
    async def test_without_context(self, mock_km_client, valid_coordinates):
        ...
        with patch(
            ...
            result = await km_interface_automation(
                operation="click", coordinates=valid_coordinates, ctx=None
            )
...
class TestInterfaceSecurity:
    ...
    @pytest.mark.asyncio
    async def test_coordinate_bounds_validation(self, mock_context, mock_km_client):
    ...
    async def test_negative_coordinate_bounds(self, mock_context, mock_km_client):
        ...
        with patch(
            ...
            result = await km_interface_automation(
                operation="click", coordinates={"x": -10000, "y": 100}, ctx=mock_context
            )
            assert result["success"] is False
            assert "out of reasonable range" in result["error"]["details"]

    @pytest.mark.asyncio
    async def test_text_length_limits(self, mock_context, mock_km_client):
        ...
        with patch(
            ...
            result = await km_interface_automation(
                operation="type", text=long_text, ctx=mock_context
            )
    ...
    @pytest.mark.asyncio
    async def test_keystroke_validation(self, mock_context, mock_km_client):
...
class TestInterfacePropertyBased:
    ...
    def test_coordinate_validation_property(self, coordinates):
    ...
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
    def test_operation_validation_property(self, operation):
...
class TestInterfacePerformance:
    ...
    @pytest.mark.asyncio
    async def test_typing_performance(self, mock_context, mock_km_client):
        ...
        for _description, length in test_cases:
            ...
            with patch(
                ...
                result = await km_interface_automation(
                    operation="type", text=text, ctx=mock_context
                )
    ...
    @pytest.mark.asyncio
    async def test_drag_operation_performance(self, mock_context, mock_km_client):
...
class TestInterfaceEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_zero_coordinates(self, mock_context, mock_km_client):
        ...
        with patch(
            ...
            result = await km_interface_automation(
                operation="click", coordinates=zero_coords, ctx=mock_context
            )
    ...
    @pytest.mark.asyncio
    async def test_negative_valid_coordinates(self, mock_context, mock_km_client):
        ...
        with patch(
            ...
            result = await km_interface_automation(
                operation="click", coordinates=negative_coords, ctx=mock_context
            )
    ...
    @pytest.mark.asyncio
    async def test_empty_text_typing(self, mock_context, mock_km_client):
        ...
        with patch(
            ...
            result = await km_interface_automation(
                operation="type", text="", ctx=mock_context
            )
    ...
    @pytest.mark.asyncio
    async def test_unicode_text_typing(self, mock_context, mock_km_client):
        ...
        with patch(
            ...
            result = await km_interface_automation(
                operation="type", text=unicode_text, ctx=mock_context
            )
    ...
    @pytest.mark.asyncio
    async def test_special_key_combinations(self, mock_context, mock_km_client):
    ...
    @pytest.mark.asyncio
    async def test_maximum_drag_distance(self, mock_context, mock_km_client):
