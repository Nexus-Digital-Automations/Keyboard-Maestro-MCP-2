...
class TestKMDetectObjects:
    ...
    @pytest.mark.asyncio
    async def test_detect_objects_comprehensive(self, mock_context, sample_image_data):
        ...
        if result["success"]:
            ...
            if isinstance(result["error"], str):
                ...
                assert "not initialized" in result["error"] or "failed" in result["error"] or "vision" in result["error"]
            else:
                # Structured error format
                assert "code" in result["error"]
                assert result["error"]["code"] in ["INITIALIZATION_ERROR", "SERVICE_UNAVAILABLE", "DETECTION_ERROR"]

    @pytest.mark.asyncio
    async def test_detect_objects_with_confidence_threshold(self, mock_context, sample_image_data):
    ...
    @pytest.mark.asyncio
    async def test_detect_objects_invalid_confidence(self, mock_context, sample_image_data):
        ...
        if isinstance(result["error"], str):
            ...
            initialization_error = "not initialized" in error_lower or "failed" in error_lower
            validation_error = "confidence" in error_lower or "invalid" in error_lower
            assert initialization_error or validation_error
        else:
            assert "code" in result["error"]
            assert result["error"]["code"] in ["VALIDATION_ERROR", "INVALID_CONFIDENCE", "INPUT_ERROR", "INITIALIZATION_ERROR"]

    @pytest.mark.asyncio
    async def test_detect_objects_empty_image_data(self, mock_context):
        ...
        if isinstance(result["error"], str):
            error_lower = result["error"].lower()
            initialization_error = "not initialized" in error_lower or "failed" in error_lower
            validation_error = "image" in error_lower or "data" in error_lower or "empty" in error_lower
            assert initialization_error or validation_error
        else:
            assert "code" in result["error"]
            assert result["error"]["code"] in ["VALIDATION_ERROR", "INVALID_IMAGE", "INPUT_ERROR", "INITIALIZATION_ERROR"]

    @pytest.mark.asyncio
    async def test_detect_objects_invalid_max_detections(self, mock_context, sample_image_data):
        ...
        if isinstance(result["error"], str):
            error_lower = result["error"].lower()
            initialization_error = "not initialized" in error_lower or "failed" in error_lower
            validation_error = "max" in error_lower or "detections" in error_lower or "invalid" in error_lower
            assert initialization_error or validation_error
        else:
            assert "code" in result["error"]
            assert result["error"]["code"] in ["VALIDATION_ERROR", "INVALID_MAX", "INPUT_ERROR", "INITIALIZATION_ERROR"]
...
class TestKMAnalyzeScene:
    ...
    @pytest.mark.asyncio
    async def test_analyze_scene_comprehensive(self, mock_context, sample_image_data):
        ...
        if result["success"]:
            ...
            if isinstance(result["error"], str):
                ...
                assert result["error"]["code"] in ["INITIALIZATION_ERROR", "SERVICE_UNAVAILABLE", "ANALYSIS_ERROR"]

    @pytest.mark.asyncio
    async def test_analyze_scene_with_emotions(self, mock_context, sample_image_data):
    ...
    @pytest.mark.asyncio
    async def test_analyze_scene_invalid_level(self, mock_context, sample_image_data):
        ...
        if isinstance(result["error"], str):
            ...
            assert result["error"]["code"] in ["VALIDATION_ERROR", "INVALID_LEVEL", "INPUT_ERROR", "INITIALIZATION_ERROR"]
...
class TestKMClassifyImageContent:
    ...
    @pytest.mark.asyncio
    async def test_classify_image_content_comprehensive(self, mock_context, sample_image_data):
        ...
        if result["success"]:
            ...
            if isinstance(result["error"], str):
                ...
                assert result["error"]["code"] in ["INITIALIZATION_ERROR", "SERVICE_UNAVAILABLE", "CLASSIFICATION_ERROR"]

    @pytest.mark.asyncio
    async def test_classify_image_content_invalid_threshold(self, mock_context, sample_image_data):
        ...
        if isinstance(result["error"], str):
            ...
            assert result["error"]["code"] in ["VALIDATION_ERROR", "INVALID_THRESHOLD", "INPUT_ERROR", "INITIALIZATION_ERROR"]
...
class TestKMExtractTextFromImage:
    ...
    @pytest.mark.asyncio
    async def test_extract_text_from_image_comprehensive(self, mock_context, sample_image_data):
        ...
        if result["success"]:
            ...
            if isinstance(result["error"], str):
                ...
                assert result["error"]["code"] in ["INITIALIZATION_ERROR", "SERVICE_UNAVAILABLE", "OCR_ERROR"]

    @pytest.mark.asyncio
    async def test_extract_text_invalid_mode(self, mock_context, sample_image_data):
        ...
        if isinstance(result["error"], str):
            ...
            assert result["error"]["code"] in ["VALIDATION_ERROR", "INVALID_MODE", "INPUT_ERROR", "INITIALIZATION_ERROR"]
...
class TestKMComputerVisionMetrics:
    ...
    @pytest.mark.asyncio
    async def test_computer_vision_metrics_complete(self, mock_context):
        ...
        if result["success"]:
            ...
            assert "system_performance" in result["metrics"] or "vision_system" in result["metrics"]
            assert "metadata" in result
        else:
            # Handle initialization or service issues
            assert "error" in result
            if isinstance(result["error"], str):
                ...
                assert result["error"]["code"] in ["INITIALIZATION_ERROR", "SERVICE_UNAVAILABLE", "METRICS_ERROR"]

    @pytest.mark.asyncio
    async def test_computer_vision_metrics_performance(self, mock_context):
